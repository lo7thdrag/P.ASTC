...
type
  ...
  private
    ...
    FOldRange : Double;
    FOnNextWaypointChange: TOnNextWaypointChange;
  ...
  public
    ...
    property OnNextWaypointChange : TOnNextWaypointChange
      read FOnNextWaypointChange write FOnNextWaypointChange;
...
uses uLibSettingTTT, {uDBAsset_radar, uDBAsset_Sonar, uDBAsset_Sensor,}
  ...
  uT3Missile, uT3Vehicle, uT3SimManager, uT3Radar, uT3Sonar,
...
begin
  ...
  begin
   tmp := FWPList.Items[FNextNodeID];
    ...
    if Assigned(FOnNextWaypointChange) then
      FOnNextWaypointChange(FNextNodeID, TT3Vehicle(FOwner).InstanceIndex,
        TScripted_Behav_Definition(tmp^.Behav).FData.Speed,
        bearing, TScripted_Behav_Definition(tmp^.Behav).FData.Waypoint_Latitude,
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if Assigned(FOnNextWaypointChange) then
                FOnNextWaypointChange(FNextNodeID, TT3Vehicle(FOwner).InstanceIndex,
                  NextWaypoint.FData.Speed, bearing, TScripted_Behav_Definition(FPNextWaypoint^.Behav).FData.Waypoint_Longitude,
          ...
          begin
            ...
            if Assigned(FOnNextWaypointChange) then
              FOnNextWaypointChange(FNextNodeID, TT3Vehicle(FOwner).InstanceIndex, 0,
                TT3Vehicle(FOwner).Heading, TScripted_Behav_Definition(FPNextWaypoint^.Behav).FData.Waypoint_Longitude,
                TScripted_Behav_Definition(FPNextWaypoint^.Behav).FData.Waypoint_Latitude);
