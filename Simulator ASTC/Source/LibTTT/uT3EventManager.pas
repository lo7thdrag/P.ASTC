...
uses tttData, Classes, uT3Torpedo, uT3Missile, uT3Unit, uSimObjects, uT3Weapon,
    uT3Vehicle, SYsUtils, uGameData_TTT, uT3Listener, uT3CounterMeasure,
...
type
  ...
  public
    ...
    procedure OnAfterLanding(Sender : TObject); virtual;
	  ...
	  //added by choco
    procedure OnWeaponHitByServer(SenderID, TargetID, Lethality : integer) ;    virtual;
    ...
    procedure OnNextWaypointChange(WaypointID, aShipID: Integer; aSpeed,
      aBearing, PosX, PosY: Double); virtual;
...
procedure TT3EventManager.OnNextWaypointChange(WaypointID, aShipID: Integer;
  aSpeed, aBearing, PosX, PosY: Double);
begin
//
end;
...
begin
  if Assigned(OnLogEventStr) then
    OnLogEventStr('TT3EventManager.OnDeploySonar', 'Sonobuoy Deployed');
end;
...
begin
  OnLogEventStr('TT3EventManager.OnECMJammingFailed', TT3DefensiveJammerOnVehicle(Sender).InstanceName
...
begin
  OnLogEventStr('TT3EventManager.OnECMJammingSuccess', TT3DefensiveJammerOnVehicle(Sender).InstanceName
...
var
  ...
  vm : TT3PlatformInstance;
  i : integer;
begin

  { free vehicle from formation manager }
  if Sender is TT3Vehicle then
  begin
    with TT3Vehicle(Sender) do
    begin
      { jika leader maka release dlu kendali platform leader di follower }
      if isFormationLeader then
      begin
        fm := SimManager.Scenario.Formation_List_rev.getByLeaderIndex(InstanceIndex);
        if Assigned(fm) then
        begin
          l := fm.getAllMember;
          ...
          begin
            fa := l.Items[I];
            vm := SimManager.FindT3PlatformByID( fa.Platform_Instance_Index );
            if Assigned(vm) then
            begin
              TT3Vehicle(vm).LeaderPlatform := nil;
              TT3Vehicle(vm).FormationModeActive := False;
              TT3Vehicle(vm).VehicleDefinition.GuidanceType := vgtStraightLine;
            end;
          end;
        end;
      end;

      SimManager.Scenario.Formation_List_rev.RemoveMemberFromFormation(InstanceIndex);
    end;
...
begin
  ...
  if Assigned(OnLogEventStr) then
    OnLogEventStr('TT3EventManager.OnPropGameSettingBoolChange', 'Game Setting ID : ' + IntToStr(Byte(Props)) + ' set to ' + BoolToStr(Value));
...
begin
  ...
  if Assigned(OnLogEventStr) then
    OnLogEventStr('TT3EventManager.OnPropGameSettingByteChange', 'Game Setting ID : ' + IntToStr(Byte(Props)) + ' set to ' + IntToStr(Value));
...
begin
  ...
  if Assigned(OnLogEventStr) then
    OnLogEventStr('TT3EventManager.OnPropGameSettingDoubleChange', 'Game Setting ID : ' + IntToStr(Byte(Props)) + ' set to ' + FloatToStr(Value));
...
begin
  ...
  if Assigned(OnLogEventStr) then
    OnLogEventStr('TT3EventManager.OnPropGameSettingIntChange', 'Game Setting ID : ' + IntToStr(Byte(Props)) + ' set to ' + IntToStr(Value));
end;
...
begin
  if Value then begin
    OnLogEventStr('TT3EventManager.OnRadarNoiseJammingSuccess', TT3RadarNoiseJammerOnVehicle(Sender).InstanceName
...
begin
  if (not(Mode = sopDeploying)) and (not(Mode = sopStowing)) then
    if Assigned(OnLogEventStr) then
      OnLogEventStr('TT3EventManager.OnSensorOperationalStatus', 'Sensor ' + TT3Sensor(Sender).InstanceName + ', Status : ' + SensorOpStatusToString(Mode));
end;
...
procedure TT3EventManager.OnWeaponHitByServer(SenderID, TargetID, Lethality : integer) ;
begin
end;
...
begin
  ...
  begin
    ...
    if (Sender is TT3Weapon) and Assigned(OnLogEventStr) then
      OnLogEventStr('TT3EventManager.OnWeaponLaunched', TT3Weapon(Sender).InstanceName + ' launch to target ' + name
                    + #13#10 + FormatFloat('0.00', TT3Weapon(Sender).Altitude * C_Meter_To_Feet));
  end
  else
  begin
    if (Sender is TT3Weapon) and Assigned(OnLogEventStr) then
      OnLogEventStr('TT3EventManager.OnWeaponLaunched', TT3Weapon(Sender).InstanceName + ' launch without target');
  end;
end;
