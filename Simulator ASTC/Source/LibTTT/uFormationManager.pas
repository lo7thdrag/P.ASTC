...
type
  ...
  public
    ...
    function RemoveMemberFromFormation(index : Integer): integer;
    ...
    function getByLeaderIndex(aIndex : Integer): TFormationRefine;
...
function TFormationManager.getByLeaderIndex(aIndex: Integer): TFormationRefine;
...
{
  remove member from any formations that exist;
  if vehicle is leader then remove formation
  if vehicle is follower then remove it from formation

  result :
  0 = leader is remove, formation removed
  1 = member is removed
  2 = vehicle not in any formation
}
function TFormationManager.RemoveMemberFromFormation(index : Integer): integer;
var
  i, j : integer;
  formation: TFormationRefine;
  fa : TClassFormation_Assignment;
begin
  Result := 2;
  ...
  begin
    ...
    formation := FFormations.Items[I];

    { if vehicle is leader }
    if formation.FFormation_Def.Formation_Leader = index then
    begin
      Result := 0;
      FFormations.Delete(I);
    ...
    { if vehicle is member }
    begin
      if formation.RemoveMemberByIndex(index) then
        Result := 1;
    end;
  end;
end;
