...
uses
  ...
  uT3DefensiveJammer ,uGameData_TTT, uSimObjects, System.ImageList;
...
begin
  ...
  case TSpeedButton(Sender).Tag of
    ...
    begin
      ...
      begin
        frmTacticalDisplay.fmMapWindow1.Map.CurrentTool := mtAddWaypoint;
        FAddMode := true;
      end
      else
      begin
...
begin
  ...
  frmTacticalDisplay.fmMapWindow1.Map.CurrentTool := miArrowTool;
...
begin
  frmTacticalDisplay.fmMapWindow1.Map.CurrentTool := mtEditWaypoint;
end;
...
begin
  ...
  begin
    ...
    if value > v.Mover.MaxSpeed then
      value := v.Mover.MaxSpeed
    else if value < v.Mover.MinSpeed then
...
procedure TfrmWaypointEditor.FormDeactivate(Sender: TObject);
begin
  AlphaBlend := True;
  AlphaBlendValue := 200;
end;
...
begin
  ...
  begin
    ...
    behav := TScripted_Behav_Definition(TT3Vehicle(FHookedPlatform).Waypoints.Behaviour[item.Index]);
    FFocusedBehav := behav;
