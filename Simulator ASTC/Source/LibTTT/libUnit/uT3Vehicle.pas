...
uses uT3Unit, Classes, u2DMover, uT3Weapon, Graphics, uObjectVisuals, SysUtils,
     uWaypoint, uSteppers, uDBAsset_Vehicle, tttData,
     uGameData_TTT, uSimObjects, ExtCtrls, math , uCalculationEnvi,
     {Hambali} uDynamicOverlay, uCoordConvertor, uSnapshotData,uT3UnitContainer;
...
//==============================================================================
  ...
  public
    ...
    function AddQueue(aSpeed, aHeading : double; aTrackID, aLaunchName : string; timeToLaunch : Double) : boolean;
...
//=============================================================================
  ...
  private
    ...
    FLastEMCON_UHFVHFDLink : TEMCON_UHFVHFDataLinkVehicle;//Boolean;
  ...
  protected
    ...
    function getTrackLabel: string; override;
    procedure setTrackLabel(const Value: string); override;
...
//    OnGuidanceStationReached        : TOnStationReached;
//    OnGuidanceStationUpdateSpeed    : TOnStationUpdateSpeed;
//    OnGuidanceStationUpdateHeading  : TOnStationUpdateHeading;
  ...
  TT3NonRealVehicle = class(TT3Vehicle)
    private
      ...
      procedure setFSymbolType(const value : String);
    ...
    public
      ...
      procedure UpdateGrowNRP(TimeUpdate, TimeNow : TDateTime);

      property NRPType : Integer read FNRPType write setFNRPType;
      property SymbolType : String read FSymbolType write setFSymbolType;
...
//==================================================
  ...
  filterType := TPictureFilterType(PlatformDomain + 1);
  ...
  FTacticalSymbol.TextVisual.Color    := FTacticalSymbol.Color; //mk
...
//  if isFormationLeader then
  ...
  //Hambali
  if ListDynamicOverlay.Count > 0 then
  begin
    for i := 0 to ListDynamicOverlay.Count - 1 do
    begin
      DynOver := TDynamicOverlay(ListDynamicOverlay.Items[i]);
      DynOver.Draw(aCnv);
    end;
  end;
end;
...
begin
  ...
  FMover.Speed        := UnitActivation.Init_Ground_Speed;
  ...
  Health :=  TVehicle_Definition(UnitDefinition).FData.Damage_Capacity -
             UnitActivation.Damage;
  ...
  case UnitActivation.Init_Guidance_Type of
    0:  // straight/heading
      begin
        ...
        OrderedSpeed := UnitActivation.Init_Ground_Speed;
      end;
    1: // waypoint
    ...
    //TVehicle_Definition(UnitDefinition).GuidanceType := vgtFormation;
  end;
...
//    OnLogEventStr(FloatToStr(speed));
...
//    Min_Ground_Speed := UnitMotion.FData.Min_Ground_Speed ;
  ...
  if (Vehicle.GuidanceType = vgtStraightLine)
      ...
      or (Vehicle.GuidanceType = vgtFormation) then
  begin
...
begin
  inherited;
  FTacticalSymbol.TextVisual.Text := Value;
end;
...
//  dt := Abs(FOrderedSpeed - FMover.Speed);
//  if dt <= 0.1 then
//    FOrderedSpeed := FMover.Speed;
...
begin // untuk 'detected' track
  ...
  if FileExists(sName) then begin
    ...
    FTacticalSymbol.TextVisual.Color := FTacticalSymbol.Color;
    FTacticalSymbol.SpeedVector.Color := FTacticalSymbol.Color;
  end;
end;

function TT3Vehicle.getTrackLabel: string;
begin
  result := FTacticalSymbol.TextVisual.Text;
end;

procedure TT3Vehicle.setTrackLabel(const Value: string);
begin
  FTacticalSymbol.TextVisual.Text := Value;
end;
...
var
    form_X, form_Y, dist, leader_baering,
    dist_to_slow, time_to_stop, speed_now : Double;
begin
//  if isFormationLeader then
//  begin
//    SetOrderedHeading(OrderedHeading);
...
//  begin
  ...
  begin
   { perhitungan GLBB, sesuai dengan decelaration masing2 vehicle.
    ...
    begin
      leader_baering := LeaderPlatform.heading{Course} + FormationBearing;
      if leader_baering > 360 then
        leader_baering := leader_baering - 360;
    end
    else leader_baering := FormationBearing;

    // finding the formation points
    CalcForward(LeaderPlatform.PosX, LeaderPlatform.PosY, FormationRange,
                leader_baering, form_X, form_Y);

    if (form_X <> 0) and (form_Y <> 0) then
    begin
      dist := CalcRange(form_X, form_Y, PosX, PosY);

      if LeaderPlatform.Speed = 0 then
      begin
        SetOrderedHeading(OrderedHeading);

        if (lastStationPointX <> LeaderPlatform.PosX)
           and (lastStationPointY <> LeaderPlatform.PosY) then
        begin
          if dist > dist_to_slow then
          begin
            OrderedHeading  := CalcBearing(PosX, PosY, form_X, form_Y);
            OrderedSpeed := UnitMotion.FData.Max_Ground_Speed;
          end
          else
          begin
            OrderedSpeed   := 0;
            SetOrderedHeading(OrderedHeading);
            //OrderedHeading := LeaderPlatform.Heading;

            lastStationPointX := LeaderPlatform.PosX;
            lastStationPointY := LeaderPlatform.PosY;
          end;
...
//              SetOrderedHeading(OrderedHeading);
//
//              FlastStationPointX := form_X;
//              FlastStationPointY := form_Y;
//            end
//            else
//            begin
//              OrderedSpeed := LeaderPlatform.Speed;
//              SetOrderedHeading(OrderedHeading);
//            end;
//          end;
        end;
...
//            if Speed <> LeaderPlatform.Speed then
//              OrderedSpeed   := LeaderPlatform.Speed;
//
//            if Heading <> LeaderPlatform.Heading then
//              OrderedHeading := LeaderPlatform.Heading;
...
//    if CalcRange(form_X, form_Y, PosX, PosY) < 0.05 then
//    begin
//      OrderedSpeed  := LeaderPlatform.Speed;
//      OrderedHeading  := LeaderPlatform.heading; //LeaderPlatform.Course;
//    end
//    else begin
//      OrderedSpeed  := Mover.MaxSpeed;
//      OrderedHeading  := CalcBearing(PosX,PosY,form_X,form_Y);
//
////      FDeltaHead := getDeltaH(FOrderedHeading, Course);
//      FDeltaHead := getDeltaH(FOrderedHeading, Heading);
...
begin
  // vehicle
  ss.Heading := Heading ;               // actual heading
  ss.GroundSpeed := GroundSpeed ;       // actual speed

  ss.WindDirection := WindDirection ;
  ss.WindSpeed     := WindSpeed ;

  if EmbarkedVehicles <> nil then begin
    SetLength(ss.EmbarkedVehicles,EmbarkedVehicles.Count);
    for I := 0 to EmbarkedVehicles.Count - 1 do begin
      embark := EmbarkedVehicles.Items[i] ;
      if embark <> nil then begin
        ss.EmbarkedVehicles[i].LaunchAlert         := embark.LaunchAlert ;
        ss.EmbarkedVehicles[i].Quantity            := embark.Quantity ;
        ss.EmbarkedVehicles[i].Availabel           := embark.Availabel ;
        ss.EmbarkedVehicles[i].Readying            := embark.Readying ;
        ss.EmbarkedVehicles[i].EmbarkedVehicle     := embark.EmbarkedVehicle ;
        ss.EmbarkedVehicles[i].EmbarkedIdentifier  := embark.EmbarkedIdentifier ;
        ss.EmbarkedVehicles[i].EmbarkDefinition    := embark.EmbarkDefinition.FData;
        //LaunchQueue         : TList ;
        ss.EmbarkedVehicles[i].IsAfterLanding      := embark.IsAfterLanding ;

        if embark.LaunchQueue.Count > 0 then
        begin
          SetLength(ss.EmbarkedVehicles[i].LaunchQueue,embark.LaunchQueue.Count);
          for k := 0 to embark.LaunchQueue.Count - 1 do
          begin
            Queue := embark.LaunchQueue.Items[k];
            if Queue <> nil then
            begin
              ss.EmbarkedVehicles[i].LaunchQueue[k].ReadyingTime := Queue.ReadyingTime;
              ss.EmbarkedVehicles[i].LaunchQueue[k].InitialSpeed := Queue.InitialSpeed;
              ss.EmbarkedVehicles[i].LaunchQueue[k].InitialHeading := Queue.InitialHeading;
              ss.EmbarkedVehicles[i].LaunchQueue[k].TrackID := Queue.TrackID;
              ss.EmbarkedVehicles[i].LaunchQueue[k].LaunchName := Queue.LaunchName;
            end;
          end;
        end;

        if embark.LandQueue.Count > 0 then
        begin
          SetLength(ss.EmbarkedVehicles[i].LandQueue,embark.LandQueue.Count);
          for k := 0 to embark.LandQueue.Count - 1 do
          begin
            Queue := embark.LandQueue.Items[k];
            if Queue <> nil then
            begin
              ss.EmbarkedVehicles[i].LandQueue[k].ReadyingTime := Queue.ReadyingTime;
              ss.EmbarkedVehicles[i].LandQueue[k].InitialSpeed := Queue.InitialSpeed;
              ss.EmbarkedVehicles[i].LandQueue[k].InitialHeading := Queue.InitialHeading;
              ss.EmbarkedVehicles[i].LandQueue[k].TrackID := Queue.TrackID;
              ss.EmbarkedVehicles[i].LandQueue[k].LaunchName := Queue.LaunchName;
            end;
          end;
        end;
      end;
  ...
  // SpeedState       : TMoveChangeState read FSpeedState;
  // HeadingState     : TMoveChangeState read FHeadingState;
  // AltitudeState    : TMoveChangeState read FAltitudeState;
  ...
  if VehicleDefinition.Sonobuoy <> nil then begin
     SetLength(ss.VehicleDefinition.FSonobuoyOnBoard,VehicleDefinition.Sonobuoy.Count);
     for I := 0 to VehicleDefinition.Sonobuoy.Count - 1 do  begin
        with  TSonobuoy_On_Board(VehicleDefinition.Sonobuoy[i]) do begin
  ...
  if VehicleDefinition.Point_Effects <> nil then begin
    SetLength(ss.VehicleDefinition.FPointEffectsOnBoard,VehicleDefinition.Point_Effects.Count);
    for I := 0 to VehicleDefinition.Point_Effects.Count - 1 do  begin
        with  TPoint_Effect_On_Board(VehicleDefinition.Point_Effects[i]) do begin
            ...
            for j := 0 to FPattern_Point.Count - 1 do  begin
  ...
  if VehicleDefinition.Towed_Jammer_Decoys <> nil then begin
    SetLength(ss.VehicleDefinition.FTowedJammerDecoyOnBoard,VehicleDefinition.Towed_Jammer_Decoys.Count);
    for I := 0 to VehicleDefinition.Towed_Jammer_Decoys.Count - 1 do  begin
        with  TTowed_Jammer_Decoy_On_Board(VehicleDefinition.Towed_Jammer_Decoys[i]) do begin
  ...
  if VehicleDefinition.Infrared_Decoys <> nil then begin
    SetLength(ss.VehicleDefinition.FInfraredecoyOnBoard,VehicleDefinition.Infrared_Decoys.Count);
    for I := 0 to VehicleDefinition.Infrared_Decoys.Count - 1 do  begin
        with  TInfrared_Decoy_On_Board(VehicleDefinition.Infrared_Decoys[i]) do begin
            ...
            begin
              ...
              for j := 0 to FScripted_Infra.Count - 1 do  begin
                scriInfra := TScripted_Infrared(FScripted_Infra[j]);
                if scriInfra <> nil then begin
            ...
            begin
              ...
              for j := 0 to FPattern_Infra.Count - 1 do  begin
                pattInfra := TPattern_Infrared(FPattern_Infra[j]);
                if pattInfra <> nil then begin
  ...
  if VehicleDefinition.Defensive_Jammers <> nil then begin
    SetLength(ss.VehicleDefinition.FDefensiveJammerOnBoard,VehicleDefinition.Defensive_Jammers.Count);
    for I := 0 to VehicleDefinition.Defensive_Jammers.Count - 1 do  begin
        with  TDefensive_Jammer_On_Board(VehicleDefinition.Defensive_Jammers[i]) do begin
...
begin
  // vehicle
  Heading       := ss.Heading ;           // actual heading
  GroundSpeed   := ss.GroundSpeed ;       // actual speed

  WindDirection := ss.WindDirection ;
  WindSpeed     := ss.WindSpeed ;

  for i := low(ss.EmbarkedVehicles) to High(ss.EmbarkedVehicles) do begin
     emb := GetEmbarkedVehicle( ss.EmbarkedVehicles[i].EmbarkedIdentifier) ;
     if emb <> nil then begin
//        emb.LaunchAlert     := ss.EmbarkedVehicles[i].LaunchAlert ;
//        emb.Quantity        := ss.EmbarkedVehicles[i].Quantity ;
        emb.Availabel       := ss.EmbarkedVehicles[i].Availabel ;
        //emb.Readying      := ss.EmbarkedVehicles[i].Readying ;
//        emb.EmbarkedVehicle := ss.EmbarkedVehicles[i].EmbarkedVehicle ;
        emb.IsAfterLanding  := ss.EmbarkedVehicles[i].IsAfterLanding ;
        emb.EmbarkedIdentifier := ss.EmbarkedVehicles[i].EmbarkedIdentifier;
        if emb.EmbarkDefinition = nil then
          emb.EmbarkDefinition := TVehicle_Definition.Create;

        emb.EmbarkDefinition.FData := ss.EmbarkedVehicles[i].EmbarkDefinition;

        for j := Low(ss.EmbarkedVehicles[i].LaunchQueue) to High(ss.EmbarkedVehicles[i].LaunchQueue) do
        begin
          emb.Quantity := emb.Quantity + 1;
          emb.AddQueue(ss.EmbarkedVehicles[i].LaunchQueue[j].InitialSpeed,
                       ss.EmbarkedVehicles[i].LaunchQueue[j].InitialHeading,
                       ss.EmbarkedVehicles[i].LaunchQueue[j].TrackID,
                       ss.EmbarkedVehicles[i].LaunchQueue[j].LaunchName,
                       ss.EmbarkedVehicles[i].LaunchQueue[j].ReadyingTime)
        end;

        for j := Low(ss.EmbarkedVehicles[i].LandQueue) to High(ss.EmbarkedVehicles[i].LandQueue) do
        begin
          emb.AddQueueLand(ss.EmbarkedVehicles[i].LandQueue[j].TrackID,
                       ss.EmbarkedVehicles[i].LandQueue[j].LaunchName,
                       ss.EmbarkedVehicles[i].LandQueue[j].ReadyingTime)
        end;
     end;
  end;
  ...
  FuelRemaining     := ss.FuelRemaining ;

  // DamagePercentSpeed:= ss.DamagePercentSpeed ;
  // DamageOverall     := ss.DamageOverall ;             // readnoly properti
  // Mover : T2DMover read FMover;
  ...
  // platform
  {
  UnitMotion.FData        := ss.P.UnitMotion ;
  UnitActivation          := ss.P.UnitActivation ;
  Force_Designation       := ss.P.Force_Designation;
  Track_ID                := ss.P.Track_ID ;
  PlatformType            := ss.P.PlatformType ;
  PlatformDomain          := ss.P.PlatformDomain ;
  LaunchTime              := ss.P.LaunchTime ;
  IsGroupLeader           := ss.P.IsGroupLeader ;
  Selected                := ss.P.Selected ;
  Altitude                := ss.P.Altitude ;
  Course                  := ss.P.Course ;
  Speed                   := ss.P.Speed ;
  Heading                 := ss.P.Heading;
  GroundSpeed             := ss.P.GroundSpeed ;
  //OnOrderedControl      := ss.TOnPlatformOrderedControl read FOnOrderedControl write SetOnO
  //OnLaunched            := ss.TNotifyEvent read FOnLaunched write SetOnLaunched;
  DormantTime             := ss.P.DormantTime ;
  // Dormant              := ss.P.Dormant;
  /// unit T3
  // Initialized          := ss.P.SST3Unit.Initialized;
  Environment.FData       := ss.P.SST3Unit.Environment.FData ;
  Environment.FGameArea   := ss.P.SST3Unit.Environment.FGameArea ;
  Environment.FGlobal_Conv:= ss.P.SST3Unit.Environment.FGlobal_Conv ;

  UnitGroup               := ss.P.SST3Unit.UnitGroup  ;
  //UnitDefinition        : TObject ;
  Lethality               := ss.P.SST3Unit.Lethality ;
  InstanceIndex           := ss.P.SST3Unit.InstanceIndex ;
  Selected                := ss.P.SST3Unit.Selected ;
  Controlled              := ss.P.SST3Unit.Controlled ;
  //IsRepositioning       := ss.P.SST3Unit.IsRepositioning ;   read only
  Health                  := ss.P.SST3Unit.Health ;
  HealthPercent           := ss.P.SST3Unit.HealthPercent;
  //OnDamageChanged       : TNotifyDamageChange DamageChanged write FOnDamageChanged;
  InstanceName            := ss.P.SST3Unit.InstanceName ;
  // InstanceClass        := ss. ;
  TrackNumber             := ss.P.SST3Unit.TrackNumber ;
  //OnSelected            : TnotifyEvent read FOnSelected write SetOnSelected;
  //OnControlled          : TNotifyEvent read FOnControlled write SetOnControlled;
  TrackLabel              := ss.P.SST3Unit.TrackLabel ;
  TrackDomain             := ss.P.SST3Unit.TrackDomain ;
  TrackIdent              := ss.P.SST3Unit.TrackIdent ;
  //OnLogEventStr         : TGetStrProc  read FOnLogEventStr write SetOnLogEventStr;
  //OnLogStr              : TGetStrProc  read FOnLogStr write SetOnLogStr;
  //OnFreeMe              : TOnPlatformFreeMe read FOnFreeMe write SetOnFreeMe;
  // DamageStatus         := ss. ;
  DetectabilityType       := ss.P.SST3Unit.DetectabilityType;
  FlagShowStatusDestroy   := ss.P.SST3Unit.FlagShowStatusDestroy  ;
  //OnHancur: THancurEvent ;
  isInstructor            := ss.P.SST3Unit.isInstructor ;
  Visible                 := ss.P.SST3Unit.Visible ;

  IsAssigned              := ss.P.SST3Unit.FObject.IsAssigned    ;
  PosX                    := ss.P.SST3Unit.FObject.PosX          ;
  PosY                    := ss.P.SST3Unit.FObject.PosY          ;
  PosZ                    := ss.P.SST3Unit.FObject.PosZ          ;
  }
end;
...
function TT3EmbarkedVehicle.AddQueue(aSpeed, aHeading: double; aTrackID,
         aLaunchName : string; timeToLaunch : Double) : boolean;
var
...
procedure TT3NonRealVehicle.setFSymbolType(const value : String);
begin
  FSymbolType := value;
end;
...
procedure TT3NonRealVehicle.UpdateGrowNRP(TimeUpdate, TimeNow: TDateTime);
var
  Diferent : TDateTime;
  SecStr : string;
  SecDbl : Double;
begin
  //**************************************************************************//
  // time update dihitung selisihnya berapa detik kemudian dirubah jadi string//
  // SecStr variable second dengan tipe data string selanjutnya dirubah       //
  // ke bentuk second float                                                   //
  {if TimeUpdate > TimeNow then
  begin
     Diferent := TimeUpdate - TimeNow;
     SecStr := Format('%0.2f', [Diferent*24*60*60]);
     SecDbl := StrToFloat(SecStr);
  end
  else
  begin
    Diferent := TimeNow - TimeUpdate;
    SecStr := Format('%0.2f', [Diferent*24*60*60]);
    SecDbl := StrToFloat(SecStr);
  end; }
  ...
  begin
    grow := C_NauticalMile_To_Degree * FMover.Speed * SecDbl * (1/3600);
  end;
end;
...
//  end;
  ...
  FTacticalSymbol.TextVisual.Text := IntToStr(Self.TrackNumber);
  FTacticalSymbol.TextVisual.Color:= FTacticalSymbol.Color; //mk

  if NRPType <> nrpBearing then
    FTacticalSymbol.Draw(aCnv);

  FTacticalSymbol.TextVisual.Draw(aCnv);
...
//    FPosition.Y := FMover.Y;
    ...
    if NRPType = nrpArea then
      Radius := CalcRange(PosX, PosY, FMover.X, FMover.Y + grow);
