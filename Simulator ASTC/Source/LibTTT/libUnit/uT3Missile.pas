...
begin
  ...
  bearing := CalcBearing(PosX, PosY, pf.PosX, pf.PosY);
  range := CalcRange(PosX, PosY, pf.PosX, pf.PosY);
  slantRange := Abs(Course - bearing);
  ...
  begin
    ...
    begin
      ...
      begin
        case Definition.Primary_Guide_Type of
          0,4,5: Result := range;
          1: Result := Side_Radar_Cross - (40 * slantRange); //TARH : find largest RCS after this calculation
          2: Result := Side_Infrared_Cross / slantRange; //IRH : find largest IRCS after this calculation
          3:
          begin
      ...
      begin
        case Definition.Primary_Guide_Type of
          0,4,5: Result := range;
          1: Result := Front_Radar_Cross - (40 * slantRange); //TARH : find largest RCS after this calculation
          2: Result := Front_Infrared_Cross / slantRange; //IRH : find largest IRCS after this calculation
          3:
          begin
            for i := 0 to TT3Vehicle(pf).Devices.Count - 1 do
            begin
    ...
    Result := TT3Chaff(pf).RCS - (40*calcAngleDifference(Course,bearing))
  else if (pf is TT3InfraredDecoy) and (Definition.Primary_Guide_Type = 2) then
    Result := TT3InfraredDecoy(pf).MaxInfraredIntensity /calcAngleDifference(Course,bearing)
  else if (pf is TT3FloatingDecoy) and (Definition.Primary_Guide_Type = 1) then
  begin
    if isBetween(30,150,delta) or isBetween(240,330,delta) then
      Result := TT3FloatingDecoy(pf).RCS_Side - (40 * bearing)
    else
      Result := TT3FloatingDecoy(pf).RCS_Front - (40 * bearing);
  end
  else
    Result := range;
end;
...
var
  ...
  Range, Azimuth, Elevation, FromAngle, ToAngle, MinAltitude, MaxAltitude,
  RainReductionFactor, PitchAngle, AltitudeAtCentreEnvelope : Double;
begin
  //cek pengaruh Rainfall Effect terhadap Missile Seeker Range Envelope
  RainReductionFactor := Get_RainRangeReductionFactor(
    Definition.Primary_Guide_Type, Environment.FData.Rain_Rate);

  Range := CalcRange(PosX, PosY, Target.PosX, Target.PosY);
  ...
  Elevation := Tan(Definition.Term_Guide_Elevation * C_DegToRad) * Range;

  FromAngle := AltitudeAtCentreEnvelope - Elevation;
  ToAngle := AltitudeAtCentreEnvelope + Elevation;
  isOnElevation := (TT3PlatformInstance(Target).Altitude > FromAngle) and
    (TT3PlatformInstance(Target).Altitude < ToAngle);

  Result := IsOnRange and IsOnAzimuth and IsOnElevation;
end;
...
begin
  ...
  FTacticalSymbol.TextVisual.Text := '+RBLW-1';
  FTacticalSymbol.TextVisual.Center := Point(tRBLWPointX, tRBLWPointY);
...
var
  str : string;
begin
  str := FTacticalSymbol.TextVisual.Text;
...
begin
  ...
  begin
    ...
    begin
      ...
      FTacticalSymbol.Symbol.LoadBitmap(vSymbolSetting.ImgPath + pctWayPoint + '.bmp',clRed);
      FTacticalSymbol.Symbol.Center := Point(tRBLWPointX, tRBLWPointY);
      FTacticalSymbol.TextVisual.Text := '+RBLW-' + IntToStr(i+1);
      FTacticalSymbol.TextVisual.Center := Point(tRBLWPointX, tRBLWPointY);
      FTacticalSymbol.Color := clRed;
      FTacticalSymbol.Draw(aCnv);
    end;
...
begin
  ...
  if Assigned(FMissileDefinition) then begin
    ...
    rec := nil;
    dmTTT.GetMotion_Characteristics(FMissileDefinition.FDef.Motion_Index,rec);
