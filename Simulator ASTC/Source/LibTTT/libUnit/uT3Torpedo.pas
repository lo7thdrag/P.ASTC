...
//    property Owner            : TSimObject read FOwner write SetOwner;
  ...
  public
    ...
    function OutsideRange(aObject : TSimObject) : boolean;
...
begin
  ...
  if Assigned(FTorpedoDefinition) then begin
    with FTorpedoDefinition do begin

      {Set katagori torpedo}
      case FDef.Guidance_Type of
        9  : FWeaponCategory := wcTorpedoStraigth;
        10 : FWeaponCategory := wcTorpedoActiveAcoustic;
        ...
        else FWeaponCategory := wcTorpedoStraigth;
      end;
...
function TT3TorpedoesOnVehicle.OutsideRange(aObject: TSimObject): boolean;
var
  rangetotarget : double;
begin
  result := false;

  rangetotarget := CalcRange(FPosition.X, FPosition.Y,
                             TT3PlatformInstance(aObject).getPositionX, TT3PlatformInstance(aObject).getPositionY);
  ...
  begin
    if (rangetotarget < TorpedoDefinition.FDef.Min_Range) or (rangetotarget > TorpedoDefinition.FDef.Max_Range) then
      result := True;
  end;
end;
...
begin
  ...
  {$REGION 'TipeTorpedo'}
  {(5)TorpedoStraigth, (6)TorpedoActiveAcoustic, (7)TorpedoPassiveAcoustic, (8)TorpedoWireGuided, (9) TorpedoWakeHoming,
   (10) TorpedoActivePassive, (11) TorpedoAirDropped }
  {$ENDREGION}
  projectil.TipeTorpedo     := TipeTorpedo; {untuk memastikan sj, karena data db editor tdk terpercaya}
  //-------------------------

  {Set Launch bearing -> heading peluncuran}
  projectil.LaunchBearing := LaunchBearing;

  {Set Search Radius -> radius berputarnya torpedo}
  ...
  begin
    if TorpedoDefinition.FDef.Fixed_Circle_Radius = 1 then
      ...
      projectil.SearchRadius := Round(TorpedoDefinition.FDef.Terminal_Circle_Radius * C_NauticalMile_To_Yards);
  end
  else
    projectil.SearchRadius := 0;

  {Set Search Depth -> batas bawah search torpedo}
  if (TipeTorpedo = 8) then
    projectil.SearchDepth := Round(TorpedoDefinition.FDef.Default_Depth * C_Feet_To_Meter)
  else
  begin
  ...
  {Set Safety Ceiling -> batas atas search torpedo}
  if (TipeTorpedo = 8) then
  ...
  begin
    if (TorpedoDefinition.FDef.Fixed_Ceiling_Depth = 0) then
      ...
      projectil.SafetyCeiling := Round(TorpedoDefinition.FDef.Acoustic_Torp_Ceiling_Depth * C_Feet_To_Meter);
  end;

  {Set SeekerRange -> jarak torpedo menyalakan seeker}
  if not Boolean(TorpedoDefinition.FDef.Fixed_Seeker_TurnOn_Range) then
    projectil.SeekerRange := Round(TorpedoDefinition.FDef.Seeker_TurnOn_Range * C_NauticalMile_To_Yards)
  else
  ...
  begin
    ...
    begin
      ...
      case FWeaponCategory of
        wcTorpedoActivePassive, wcTorpedoWakeHoming, wcTorpedoActiveAcoustic, wcTorpedoPassiveAcoustic, wcTorpedoWireGuided :
        begin
...
begin
  ...
  if FNeedAdjust then begin
    ...
    begin
      if (TipeTorpedo = 11) or (TipeTorpedo = 9) then
      begin
        SetOrderedHeading(LaunchBearing);
        FNeedAdjust := false;
      end
      else if (TipeTorpedo = 8)  then
      begin
        case TorpedoDefinition.FDef.Pursuit_Guidance_Type of
          2: {Lead}
          begin
...
begin
  ...
  if FLaunching then begin
    ...
    FTacticalSymbol.TextVisual.Visible  := Visible;
...
begin
  ...
  if (not (Assigned(FParent)) and (TipeTorpedo <> 2) )then  //add 26042012 mk
    Exit;

  if TipeTorpedo = 11 then
  begin
...
//  {Titik torpedo saat ini}
//  FTorLastPosition.X := FPosition.X;
//  FTorLastPosition.Y := FPosition.Y;
//  FTorLastPosition.Z := FPosition.Z;
  ...
  if isInstructor then            //edit mk 26042012
...
begin
  ...
  begin
    ...
    case FLaunchMethod of
      ...
      begin
        if rangeFromTarget < TorpedoDefinition.FDef.Seeker_TurnOn_Range then
        begin
          FSeekerOn := true;
        end;
      end;
      ...
      begin
        if rangeFromLaunch > TorpedoDefinition.FDef.Seeker_TurnOn_Range then
          FSeekerOn := true;
      end;
    end;

    if (TipeTorpedo = 2) or (TipeTorpedo = 8) or (TipeTorpedo = 11)then
    begin
      FSeekerOn := true;
    end;
...
begin
  ...
  while (i < FTargetPlatforms.itemCount) do begin
    ...
    case FWeaponCategory of
      ...
      wcTorpedoActivePassive :{APG mencari target terdekat}
      begin
      ...
      wcTorpedoWireGuided: {WH mencari target yg pertama kali terdeteksi}
      begin
        if not isWGTargetCheck(pf) then
          Continue;
      end;
    end;
...
begin
  ...
  if FLaunching then begin
    ...
    begin
      if TipeTorpedo = 2 then {Hybrid Tropedo move}
        AirDroppedMove(aDeltaMs)
      else if TipeTorpedo = 5 then  {Torpedo Straigth move}
        StraigthMove(aDeltaMs)
      else if TipeTorpedo = 6 then  {Torpedo ActiveAcoustic move}
        ActiveAcousticMove(aDeltaMs)
      else if TipeTorpedo = 7 then  {Torpedo PassiveAcoustic move}
        PassiveAcousticMove(aDeltaMs)
      else if TipeTorpedo = 8 then  {Torpedo WireGuided move}
        WireGuidedMove(aDeltaMs)
      else if TipeTorpedo = 9 then  {Torpedo WakeHoming move}
        WakeHomingMove(aDeltaMs)
      else if TipeTorpedo = 10 then {Torpedo ActivePassive move}
        ActivePassiveMove(aDeltaMs)
      else if TipeTorpedo = 11 then {Torpedo AirDropped move}
        AirDroppedMove(aDeltaMs);
    end;
...
begin
  ...
  if Value then begin
    ...
    {Kedalaman Awal}
    if Assigned(FParent) then begin
      {jk parentnya kapal selam, diset sesuai kedalaman parent}
    ...
    case Round(TipeTorpedo) of
      2 :
      begin
        Self.OrderedAltitude  := SearchDepth;
        Self.Heading          := TT3PlatformInstance(Parent).Heading;
        FTargetBearing        := TT3PlatformInstance(Parent).Heading;
      end;
      8:
      begin
        OrderedAltitude  := TT3PlatformInstance(TargetObject).Altitude;
      end;
      10:
      begin
