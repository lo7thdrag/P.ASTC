...
//    procedure CreateTT3TorpedoInstance(Pi : TPlatform_Instance);
  ...
  protected
    ...
    function InitiateEmbarkPlatform(const rec : TRecCmd_Embark ):boolean;virtual;
...
//    procedure CheckAllMineBomb(aDeltaMs : Double); virtual;
    ...
    function CreateTorpedoProjectileHybrid(const PlatformID, MisID,
    ...
    function FindNonRealPlatformByID(const id : integer) : TT3PlatformInstance;
...
begin
  ...
  begin
    ...
    (pi as TT3Vehicle).Waypoints.OnNextWaypointChange   := EventManager.OnNextWaypointChange;
    ...
    begin
      ...
      if Assigned(decoyOnVeh) and (TT3FloatingDecoyOnVehicle(decoyOnVeh).Quantity > 0) then
        with TT3FloatingDecoyOnVehicle(decoyOnVeh) do begin
          ...
          if FIsInstructor or decoy.UnitGroup then
            decoy.Visible := True
          else
            decoy.Visible := False;
...
begin
  for I := 0 to FScenario.rpList.Count - 1 do
  begin
    //set record
    ref_point := FScenario.rpList[I];
    ...
    rec.pZ      := 0;
    rec.Symbol  := getNameReferencePoint(ref_point.FData.Track_Type,rec.TrackDomain,rec.Identity);
...
function TT3SimManager.InitiateEmbarkPlatform(
  const rec: TRecCmd_Embark): boolean;
var
  papi : TT3Vehicle;
  aObj : TObject;
...
begin
  ...
  pi := CreatePlatformInstance(1, hostPlatform.EmbarkedVehicle.Hosted_Vehicle_Index, 0);
...
//    Exit;
  ...
  if hostPlatform.TempReadying = -1 then
    result := hostPlatform.AddQueue(rec.InitSpeed,rec.InitCourse,rec.TrackIdent,
              rec.LaunchName, pi.Vehicle.FData.Alert_State_Time)//.Readying_Time)
  else
    result := hostPlatform.AddQueue(rec.InitSpeed,rec.InitCourse,rec.TrackIdent,
...
//              LoadFormation(TT3Vehicle(p),  0);
            ...
            // follower remove from formation
            if rec^.OrderParam = 2 then
            begin
              TT3Vehicle(p).FormationModeActive := False;
            end;
    ...
    begin
      ...
      case rec^.OrderType of
        ...
        Byte(vgtWaypoint) : TVehicle_Definition(p.UnitDefinition).GuidanceType := vgtWaypoint;
...
var
  ...
  fm, fmSelected : TFormationRefine;
...
//    VehicleWeapon.NoCubicle := r^.NoCubicle;
    ...
    {$REGION 'TipeTorpedo'}
      {(5)TorpedoStraigth,    (6)TorpedoActiveAcoustic,  (7)TorpedoPassiveAcoustic, (8)TorpedoWireGuided,
       (9) TorpedoWakeHoming, (10) TorpedoActivePassive, (11) TorpedoAirDropped }
    {$ENDREGION}
    if (r^.TorpType = 5) or (r^.TorpType = 8) then
    begin
      ...
      begin
        case r^.Mode of
          ...
          begin
            ...
            FFocusedBehav := TScripted_Behav_Definition(OwnVehicle.Waypoints.Behaviour[r^.WaypointIndex]);
...
begin
  if Sender is TPlatform_Instance then begin
...
//    piInput.Capability.Clone(Capability);
...
begin
  ...
  begin
    ...
    if FIsInstructor or chaff.UnitGroup then //edit 26042012 mk
      chaff.Visible := True
    else
      chaff.Visible := False;
...
begin
  ...
  if FIsInstructor or Bubble.UnitGroup then //edit 26042012 mk
    Bubble.Visible := True
  else
...
//    torpedoprojectile.TargetPlatforms := SimPlatforms;
    ...
    if FIsInstructor or projectile.UnitGroup then
...
begin
  ...
  begin
    ...
    if FIsInstructor or Sonobuoy.UnitGroup then
...
begin
  ...
  case rec.Index of
    ...
    // Time format values
    GSID_WRECKTIME, GSID_FLASHTransmission..GSID_RoutineTransmission,
 ...
 if Assigned(OnLogStr) then
    OnLogStr('TT3SimManager.ChangeGameSetting', 'Change Game Setting : ' + GSINFO[rec.Index] + ' to ' + tmp);
end;
...
begin
  ...
  begin
    //Propulsion
  end else if rec.StateID = 4 then
  begin
...
//    TT3MissilesOnVehicle(mis).FiringMode := TMissileFiringMode(rec.FiringMode);
    ...
    grp := FScenario.CubiclesGroupsList.GetGroupByID(TT3PlatformInstance(projectile.Parent).GroupIndex) as T3CubicleGroup;
...
//      else
    ...
    if FIsInstructor or projectile.UnitGroup then //edit 26042012 mk
      projectile.Visible := True
    else
      projectile.Visible := False;
...
begin
  ...
  obj := FScenario.FindRuntimePlatform(vIndex);

  if obj = nil then
    Exit;

  if TPlatform_Library_Entry(obj).FData.Vehicle_Index <> 0 then begin
    FDeployedObject := CreatePlatformInstance(1, vIndex, nIndex);
...
//
  ...
  // deploy mine bulsit
  if TPlatform_Library_Entry(obj).FData.Mine_Index <> 0 then
  begin
    FDeployedObject := CreatePlatformInstance(2, vIndex, nIndex);
    with FDeployedObject do begin
      FData.Track_ID                      := rec.TrackIdent;
      FData.Instance_Name                 := rec.InstanceName;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        SearchRadius  := Round(TorpedoDefinition.FDef.Terminal_Circle_Radius * C_NauticalMile_To_Yards);
        ...
        SeekerRange   := Round(TorpedoDefinition.FDef.Seeker_TurnOn_Range * C_NauticalMile_To_Yards);
        TipeTorpedo   := 11;
      end;
      ...
      begin
        ...
        if FIsInstructor or projTorp.UnitGroup then
    ...
    begin
      ...
      begin
        ...
        if FIsInstructor or projBomb.UnitGroup then
...
begin
  ...
  grp := FScenario.CubiclesGroupsList.GetGroupByID(rec.NRPGroupID) as T3CubicleGroup;
...
begin
  ...
  begin
    ...
    for j := 0 to pi.Vehicle.Hosted_Platform.Count - 1 do
    begin
      hostPlatform  := TT3EmbarkedVehicle.Create;
      hostPlatform.Parent := vehicle;
      hostPlatform.IndexEmbark := j;
      hostPlatform.OnAfterLAnding := EventManager.OnAfterLanding;
      hostPlatform.Quantity := THosted_Platform(pi.Vehicle.Hosted_Platform[j]).FData.Quantity;
      hostPlatform.EmbarkedVehicle := THosted_Platform(pi.Vehicle.Hosted_Platform[j]).FData;
      hostPlatform.Readying := THosted_Platform(pi.Vehicle.Hosted_Platform[j]).FVehicle.Readying_Time;
//      hostPlatform.EmbarkedIdentifier := getIdentifier(THosted_Platform(pi.Vehicle.Hosted_Platform[j]).FData.Hosted_Vehicle_Index,
//                                         TVehicle_Definition);
      hostPlatform.EmbarkedIdentifier := THosted_Platform(pi.Vehicle.Hosted_Platform[j]).FVehicle.Vehicle_Identifier;
      hostPlatform.EmbarkDefinition := getVehicleDefinition(THosted_Platform(pi.Vehicle.Hosted_Platform[j]).FData.Hosted_Vehicle_Index);

      TT3Vehicle(vehicle).addEmbarked(hostPlatform);
    end;
  end;

//   jika platform instance tipe lainnya??
//  if pi.FData.Missile_Index <> 0 then
//    CreateTT3MissileInstance(pi);

//  if pi.FData.Torpedo_Index <> 0 then
//    CreateTT3TorpedoInstance(pi);
...
begin
  ...
  begin
    ...
    if Assigned(OnLogStr) then
       OnLogStr('TT3SimManager.CreateNonRealPlatformInstance', 'Default TPlatform_Instance for Non Real Platform = '
                      + FScenario.DefaultNonRealPlatform.Vehicle.FData.Vehicle_Identifier);
  end
  else
  begin
    if Assigned(OnLogStr) then
       OnLogStr('TT3SimManager.CreateNonRealPlatformInstance', 'Default TPlatform_Instance for Non Real Platform = nil');
