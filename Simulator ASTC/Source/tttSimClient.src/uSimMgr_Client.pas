...
uses
  ...
  uGameData_TTT, uNetSessionData, uSnapshotRec,
  uSteppers, uT3UnitContainer, uT3Unit,
  uSimObjects, uT3GroupList, uT3Vehicle, uDBCubicles,
  uDrawOverlay, uT3Weapon, uT3counterMeasure, uT3Missile,
  uT3Gun, uT3Torpedo, uT3Bomb, uT3Mine, uT3Visual, uDBAsset_Scripted,
  uT3DetectedTrack, uT3DataLink, uT3Sensor, uDBAssetObject,
  uT3MissileDetail, uT3MissileEnvironment, newClassASTT, uExecuter, uCommTTT,
  Registry, MMSystem, uDBAsset_Vehicle, uSnapshotData, uT3Sonobuoy, uMainOverlay,
  uT3DefensiveJammer, tttData, Dialogs, uDataTypes;
...
type
  ...
  protected
    ...
    // FIsInstructor   : boolean; -- sementara ta pindah ke T3simmanager, soalnya utk visibility obyek yg dibuat
  ...
  private
    ...
    //procedure InitVehicleAsGroup(v: TT3Vehicle);
...
//    procedure netSend_CmdTorpedoSync(r: TRecTorpedoSync);
  protected
    // receive UDP
    ...
    procedure netRecv_CmdRemovePlatfromByOperator(apRec: PAnsiChar;
              aSize: word);
  ...
  protected
    ...
    function LaunchRuntimePlatform(const rec: TRecCmd_LaunchRP)
             : TT3PlatformInstance; override;
    function LaunchReferencePoint(const rec: TRecCmd_LaunchNonRP)
             : TT3PlatformInstance; override;
    function UpdatePropertyPlatform(const rec: TRecCmd_Platform_SET_PROPERTY)
             : TT3PlatformInstance; override;
    function UpdateNRPProperty(const rec: TRecCmd_NRP_SET_PROPERTY)
             : TT3PlatformInstance;
    ...
    procedure OnBombMineHit(sender: TSimObject; Target: TSimObject;
              lethality: integer);
  public
    ...
    function FindNearestRBLWaypointTrack(const px, py: double;
       const maxDist: integer): TSimObject;
    function FindNearestBOLWaypointTrack(const px, py: double;
       const maxDist: integer): TSimObject;
  ...
  public
    ...
    DrawOverlayTemporary: TDrawOverlay;
    ...
    procedure netSend_CmdRemote(const mIP: string; const mColor: Byte;
        const mControl: Boolean; const ordID: Byte);
...
function TSimMgr_Client.AddDetectedTrack(aTrack: TSimObject;
  sensor: TSimObject; value : Boolean): Boolean;
var
  pi: TT3Unit;
...
//    TT3Unit(aTrack).TrackNumber := FTrackNumber;
    ...
    if TT3Sensor(sensor) is TT3ESMSensor then
    begin
      track.TrackDomain := 0;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FIsInstructor then FClientRole := crpInstruktur
      else FClientRole := crpCubicle;
    end;
  end;
...
begin
  ...
  VNetClient.RegisterTCPPacket(CPID_CMD_SYNC_WAYPOINTDATA, SizeOf(TRecSync_WaypointData), netRecv_Synch_WaypointData);
...
begin
  ...
  //  hw := Converter.FMap.Width div 2;
  //  hh := Converter.FMap.Height div 2;
...
begin
  ...
  // di comment dlu biar g berat cycle prosesnya
  // SimWeapons.Validate;
  // SimDevices.Validate;
  ...
  // SimPlatforms.UpdateVisuals;
  // SimPlatforms.Draws(aCnv);

  SimDevices.UpdateAndDrawVisuals(aCnv, aRect);
  // SimDevices.UpdateVisuals;
  // SimDevices.Draws(aCnv);

  SimWeapons.UpdateAndDrawVisuals(aCnv, aRect);
  // SimWeapons.UpdateVisuals;
  // SimWeapons.Draws(aCnv);

  SimDetectedTrackList.UpdateAndDrawVisuals(aCnv, aRect);
  // SimDetectedTrackList.UpdateVisuals;
  // SimDetectedTrackList.Draws(aCnv);

  simNonRealPlatform.UpdateAndDrawVisuals(aCnv, aRect);
  // simNonRealPlatform.UpdateVisuals;
  // simNonRealPlatform.Draws(aCnv);

  DrawFlagPoint.Draw(aCnv);
  DrawOverlayTemplate.Draw(aCnv, VSimMap.FMap);
  DrawOverlayTemporary.Draw(aCnv, VSimMap.FMap);
...
//      TT3Vehicle(Result).TrackNumber := grp.GetTrackNumber(rec.EmbarkPlatformID);
...
begin
  ...
  if (rec.GrpID = 0) and not ISInstructor then
    Exit;

  if ((ISInstructor) and (rec.GrpID = 0)) then
  begin
...
var
  ...
  i, J: integer;
begin
  if (ISInstructor) or (rec.GrpID = FMyCubGroup.FData.Group_Index) then
  begin
    if ISInstructor then
    begin
      // NON REAL TIME
    ...
    case rec.OrderID of
      ...
      CORD_ID_cease: // stop gun fire
        begin
          ...
          Log('TSimMgr_Client.GunFire', 'Received : ' + rec.GunName + ' - Cease Fire ');
          Exit;
        end;
...
//  fPattern := vMapSetting.PatternPath + FScenario.predefPatternName + '.dat'; ;
//  if FileExists(fPattern) then
//  begin
//    VSimMap.LoadPredefinedPattern(fPattern);
//  end;
...
begin
  ...
  try
    ...
    case tipe of
      osDynamic : DrawOverlayTemplate.ShowStreamDataDynamic(Fs, overlay);
      osStatic  : DrawOverlayTemplate.ShowStreamDataStatic(Fs, overlay);
    end;
  except
...
begin
  ...
  begin
    ...
    begin
      if ISInstructor and (rec^.GroupID = 0) then
      begin
        pi.RepositionTo(rec^.x, rec^.y);
        VSimMap.DrawMap;
      end
...
begin
  ...
  begin
    ...
    if Assigned(SelectedMember) then
    begin
      OldGroup.RemoveMember(SelectedMember.FData.Platform_Instance_Index);
      NewGroup.AddMember(SelectedMember.FData.Platform_Instance_Index);
      SelectedMember.FData.Force_Designation := NewGroup.FData.Force_Designation;

      ve := TT3Vehicle(FindT3PlatformByID(SelectedMember.FData.Platform_Instance_Index));
      if Assigned(ve) then
      begin
        ve.Force_Designation := NewGroup.FData.Force_Designation;
        ...
        if (OldGroup.FData.Group_Index = FMyCubGroup.FData.Group_Index) and
          (OldGroup.Count <= 0) then
          SimDetectedTrackList.ClearAndRemove;
...
//  Torpedo.PosZ := HybridMissile.PosZ;
  ...
  if Torpedo <> nil then
  begin
    if FIsInstructor or Torpedo.UnitGroup then
...
var
  ...
  grpID : Integer;
begin
  grp := FScenario.CubiclesGroupsList.GetGroupOf_PlatformIndex(pi_id);
//  if grp = nil then
//  Exit;
...
begin
  ...
  case aRec^.OrderID of
    CORD_ID_INITIATE:
      InitiateEmbarkPlatform(aRec^);
    CORD_ID_LAUNCH:
      begin
        pf := LaunchEmbarkPlatform(aRec^);
        if pf = nil then Exit;
        ...
        // if Assigned(pf) then pf.Controlled := true;
      end;
  end;
...
begin
  ...
  grp := FScenario.CubiclesGroupsList.GetGroupOf_PlatformIndex(r.PlatformParentID) ;
  if Assigned(grp) then
...
begin
  ...
  // if Assigned(FOnNotify_GameCtrl) then
  // FOnNotify_GameCtrl(apRec);

  {Nando Added For Recording.  GameSpeed = 1 -> Do -> Recording -}
//  if ISInstructor then
//  begin
    //EventManager.OnReceiveGameSpeedForRecord(r^);
//  end;
end;
...
begin
  ...
  if simMgrClient.ConsoleName = r^.ConsoleName then
...
begin
  ...
  case orderID of
    CPID_CMD_SNAPSHOT_SUCCESS : msg := ' "'+msg+'" success.';
    CPID_CMD_SNAPSHOT_FAIL    : msg := 'save '+msg+' failed' ;
    CPID_CMD_SNAPSHOT_EXIST   : msg := msg +' is exist' ;
  end;

  TT3ClientEventManager(EventManager).OnSnapshotEventDone(msg);
  FOnLogStr('TSimMgr_Client.netRecv_cmd_snapshot', 'Snapshot :: '+msg)
end;
...
begin
  ...
  if r.Cubicle = 0 then
  begin
    TT3ClientEventManager(EventManager).OnOverlayStaticShape(rec);
  end
  else
  begin
    if FMyCubGroup <> nil then
...
procedure TSimMgr_Client.netRecv_CmdOverlayTemplate(apRec: PAnsiChar;
  aSize: word);
var
...
begin
  ...
  if r^.Cubicle = 0 then
  begin
    TT3ClientEventManager(EventManager).OnOverlayTemplate(rec);
  end
  else
  begin
    if FMyCubGroup <> nil then
  ...
  case rec.OrderID of
    CORD_ID_SD_EVTJAMSUCCESS :
      begin
        ...
        if (not FIsInstructor) and (jammedObject is TT3Radar) then
        begin
...
begin
  ...
  if r.Cubicle = 0 then
  begin
    TT3ClientEventManager(EventManager).OnOverlayDynamicShape(rec);
  end
  else
  begin
    if FMyCubGroup <> nil then
...
//          l.Free;
      ...
      begin
        ...
        begin
          ...
          reqCubicle := Scenario.CubiclesGroupsList.GetGroupOf_PlatformIndex(r^.PooledPUID);
          ...
          if Assigned(OnLogStr) then
            OnLogStr('TSimMgr_Client.netRecv_Cmd_DataLink', 'Datalink ' + IntToStr(r^.DataLinkID) + ':' +
...
begin
  ...
  begin
    if isController then
    begin
      if i < (FScenario.CubiclesGroupsList.Count - 5) then
        Continue;
    end;
    ...
    newGrp.FData.Deployment_Index := T3CubicleGroup(FScenario.CubiclesGroupsList.Items[FScenario.CubiclesGroupsList.Count-1]).FData.Deployment_Index;
    ...
    FScenario.CubiclesGroupsList.FSList.AddObject(IntToStr(newGrp.FData.Group_Index), newGrp);
  end;

  for i := low(lRec.FRCubicleGroupList) to high(lRec.FRCubicleGroupList) do begin
    recSSCubGroup := lRec.FRCubicleGroupList[i] ;
    if recSSCubGroup.FData.Group_Identifier <> 'Controller' then begin
      grp := T3CubicleGroup.Create;
      grp.FData        := recSSCubGroup.FData ;
      grp.CubicleName  := recSSCubGroup.CubicleName;
      grp.InitData;
      for j := low(lRec.FRCubicleGroupList[i].FSList) to high(lRec.FRCubicleGroupList[i].FSList) do begin
      ...
      FScenario.CubiclesGroupsList.FSList.AddObject(IntToStr(grp.FData.Group_Index), grp);
    end;
  end;
  ...
  begin
    ...
    grp.InitData;
    FScenario.CubiclesGroupsList.FSList.AddObject(IntToStr(i), grp);
  end;

  for I := 0 to FScenario.CubiclesGroupsList.Count - 1 do
  begin
    grp  := FScenario.CubiclesGroupsList.Items[i] as T3CubicleGroup;
    if grp <> nil then
    begin
      grp.InitData;
...
begin
  ...
  if __snapshot_loadfile(fName) then begin
    ...
    FOnLogStr('TSimMgr_Client.SnapshotLoadFile', 'Snapshot :: Use snapshot '+QuotedStr(slog));
  end
  else
    FOnLogStr('TSimMgr_Client.SnapshotLoadFile', 'Snapshot Info : Error Load snapshot '+QuotedStr(fName) );
...
//  simNonRealPlatform.ClearObjectAfterSnapshot;
  ...
  FOnLogStr('TSimMgr_Client.SnapshotLoadFile', '...........................................................................');
  FOnLogStr('TSimMgr_Client.SnapshotLoadFile', 'Snapshot :: Load Success');
  FOnLogStr('TSimMgr_Client.SnapshotLoadFile', '...........................................................................');
  ...
  S := UpperCase(Trim(GameEnvironment.FGameArea.Detail_Map));
  if S = 'ENC' then
    fGeo := vMapSetting.MapENCPath + lRec.FRGameScenario.MapGeosetName
  else
    fGeo := vMapSetting.MapPath + lRec.FRGameScenario.MapGeosetName;
