...
uses
  ...
  uT3MissileDetail, uT3MissileEnvironment, ufmWeaponCtrl,
...
type
  TfmWeapon = class(TfmControlled)
    ...
    btnLaunchWH: TButton;
    EdtSalvoWH: TEdit;
    EdtLaunchBearingWH: TEdit;
    EdtSeekerRangeWH: TEdit;
    ...
    bvl3: TBevel;
    bvl4: TBevel;
    EdtAPTargetTrack: TEdit;
    EdtSafetyCeilingAP: TEdit;
    EdtSalvoAP: TEdit;
    EdtSearchDepthAP: TEdit;
    EdtSearchRadiusAP: TEdit;
    EdtSeekerRangeAP: TEdit;
    lbl1: TLabel;
    lbl12: TLabel;
    lbl13: TLabel;
    lbl14: TLabel;
    ...
    EdtLaunchBearingAD: TEdit;
    EdtSearchCeilingAD: TEdit;
    EdtSearchDepthAD: TEdit;
    EdtSearchRadiusAD: TEdit;
    lbl38: TLabel;
    lbl39: TLabel;
    lbl40: TLabel;
    lbl41: TLabel;
    ...
    procedure WakeHomingKeyPress(Sender: TObject; var Key: Char);
    procedure ActivePasiveKeyPress(Sender: TObject; var Key: Char);
    procedure AirDroppedKeyPress(Sender: TObject; var Key: Char);
    ...
    procedure chkAirDroppedUseLaunchPlatformHeadingClick(Sender: TObject);
    ...
    procedure chkAirDroppedLaunchWhithoutTargetClick(Sender: TObject);
...
//      WpnIntCont : TList;
      ...
      function TorpedoPrelaunchCheck(target,weapon : TSimObject; TipeTorpedo : Integer) : boolean;
      function TorpedoTargetCheck(target, weapon : TSimObject) : boolean;
      function BombPrelaunchCheck(target,weapon : TSimObject) : boolean;
      function BombTargetCheck(target, weapon : TSimObject) : boolean;
//      function CaclMidTarget_OwnShip(TargetX ,OwnShipX, TargetY, OwnShipY : Double): t2Dpoint;
...
begin
  ...
  {Untuk testing}
//  FAGroupBox[wcTorpedoStraigth]         := grbStraightRunningTorpedos ;
//  FAGroupBox[wcTorpedoActiveAcoustic]   := grbStraightRunningTorpedos ; //nil ;
//  FAGroupBox[wcTorpedoPassiveAcoustic]  := grbStraightRunningTorpedos ; //nil ;
//  FAGroupBox[wcTorpedoWireGuided]       := grbStraightRunningTorpedos;
//  FAGroupBox[wcTorpedoWakeHoming]       := grbStraightRunningTorpedos ;
//  FAGroupBox[wcTorpedoAirDropped]       := grbStraightRunningTorpedos ;
//  FAGroupBox[wcTorpedoActivePassive]    := grbStraightRunningTorpedos ; //nil ;
...
begin
  ...
  begin
    for i := 0 to Missile.RBLW_STOTList.ListNonSort.Count - 1 do
    begin
      RBLWPoint := Missile.RBLW_STOTList.ListNonSort.Items[i];

      if RBLWPoint.LauncherIndex <> rMis.LauncherID then
      begin
        rMis.LauncherID := RBLWPoint.LauncherIndex;
        Inc(FRippleCount);
        ...
        begin
          ...
          LauncherOn := missile.GetLauncherByID(RBLWPoint.LauncherIndex);
...
var
  ...
  strTargetTrackId, ConsoleName                          : string;

  intParentPlatformID, intNoCubicle,
  intTargetPlatformID, intbatasADT,
  intTemp                                   : Integer;
...
begin
    ...
    case TSpeedButton(Sender).Tag of
      ...
      5 : begin
            ...
            begin
              ...
              rPanel.ConsoleName := simMgrClient.ConsoleName;
              ...
              rPanel.SalvoSize := 1;
              rPanel.SearchRadius := Round(TorpedoDefinition.FDef.Terminal_Circle_Radius * C_NauticalMile_To_Yards);
  ...
  begin
    with TT3TorpedoesOnVehicle(focused_weapon) do begin
      if (EdtAPTargetTrack.Text = '') or (EdtAPTargetTrack.Text = '---') then
        Exit;
      ...
      case TButton(Sender).Tag of
        1 : begin
              ...
              SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 10, 2,
                         InstanceIndex, InstanceName, intTemp);
            end;
        2 : begin
              intTemp := Round(TorpedoDefinition.FDef.Default_Depth * C_Feet_To_Meter);
              SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 10, 3,
                         InstanceIndex, InstanceName, intTemp);
            end;
        3 : begin
              intTemp := Round(TorpedoDefinition.FDef.Acoustic_Torp_Ceiling_Depth
                         * C_Feet_To_Meter);
              SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 10, 4,
                         InstanceIndex, InstanceName, intTemp);
            end;
        4 : begin
              intTemp := Round(TorpedoDefinition.FDef.Seeker_TurnOn_Range
                         * C_NauticalMile_To_Yards);
              SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 10, 5,
                         InstanceIndex, InstanceName, intTemp);
            end;
        5 : begin
              if (focusTarget = nil) or (EdtAPTargetTrack.Text = '')then
              begin
              ...
              EdtAPTargetTrack.Text := '';
              intbatasADT := Quantity;

              if (TorpedoPrelaunchCheck(focusTarget,focused_weapon, 10)) and (intbatasADT >= (StrToInt(EdtSalvoAP.Text))) then
              begin
                if (EdtSearchRadiusAP.Text = '') or (EdtSearchDepthAP.Text = '') or
                     (EdtSafetyCeilingAP.Text = '') or (EdtSeekerRangeAP.Text = '')then
                    ShowMessage('Data Torpedo belum diisi')
                else
                begin
                  SalvoAPGGlobal := StrToInt(EdtSalvoAP.Text);
                  QuantityAPGGlobal := intbatasADT;
                  ...
                  SimMgrClient.netSend_CmdTorpedoSyncPanelWeapon(rPanel); {untuk enable button launch}
                  {------------------------------------------------------------------}
                  ...
                  lblQuantityAPG.Caption  := IntToStr(Quantity-StrToInt(EdtSalvoAP.Text));
                end;
              end
              else
              begin
                if intbatasADT <= (StrToInt(EdtSalvoAP.Text)) then
...
begin
  ...
  with TT3TorpedoesOnVehicle(focused_weapon) do begin
    case TSpeedButton(Sender).Tag of
      ...
      5 : begin
            if (chkAirDroppedLaunchWhithoutTarget.Checked = true) then
              Exit;
  ...
  begin
    with TT3TorpedoesOnVehicle(focused_weapon) do
    begin
      if (chkAirDroppedLaunchWhithoutTarget.Checked = false) then
      begin
        if (EdtADTargetTrack.Text = '') or (EdtADTargetTrack.Text = '---') then
          Exit;
      end;
      ...
      case TButton(Sender).Tag of
        1 : begin
              intTemp := Round(TorpedoDefinition.FDef.Default_Depth * C_Feet_To_Meter);
              SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 11, 2,
                         InstanceIndex, InstanceName, intTemp);
            end;
        2 : begin
              intTemp := Round(TorpedoDefinition.FDef.Acoustic_Torp_Ceiling_Depth
                         * C_Feet_To_Meter);
              SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 11, 3,
                         InstanceIndex, InstanceName, intTemp);
            end;
        3 : begin
              if ((focusTarget = nil) or (EdtADTargetTrack.Text = '')) and
                 (chkAirDroppedLaunchWhithoutTarget.Checked = False) then
              begin
                frmTacticalDisplay.addStatus('Target is not selected!');
                exit;
              end;
              ...
              if (TorpedoPrelaunchCheck(focusTarget,focused_weapon, 11)) and (intbatasADT > 0) then
              begin
                if ((EdtSearchRadiusAD.Text = '') or (EdtSearchDepthAD.Text = '') or (EdtSearchCeilingAD.Text= '')
                   or ((EdtLaunchBearingAD.Text ='')and(chkAirDroppedUseLaunchPlatformHeading.Checked=false)))
                   and (chkAirDroppedLaunchWhithoutTarget.Checked = False) then
                  ShowMessage('Data Torpedo belum diisi')
                else
                begin
                  {Paket Launch Torpedo}
                  ...
                  lblADQuantity.Caption  := IntToStr(Quantity-1);
                end;
              end
              else
              begin
                if intbatasADT <= 0 then
                  ShowMessage('Cek Quantity')
              end;
            end;
      end;
...
procedure TfmWeapon.chkAirDroppedLaunchWhithoutTargetClick(Sender: TObject);
begin
  inherited;
      ...
      //with TT3TorpedoesOnVehicle(focused_weapon) do begin
        ...
        begin
          if simMgrClient.ISInstructor then begin
            TT3TorpedoesOnVehicle(focused_weapon).NoCubicle  := 0;
          end
          else begin
            TT3TorpedoesOnVehicle(focused_weapon).NoCubicle  :=
          ...
          if FControlled is TT3PlatformInstance then
          begin
            rPanel.ParentPlatformID :=
                TT3PlatformInstance(FControlled).InstanceIndex;
            rPanel.TargetPlatformID :=
                TT3PlatformInstance(FControlled).InstanceIndex;
          end;

          rPanel.TargetTrack := '---';

          if Assigned(TT3TorpedoesOnVehicle(focused_weapon).TorpedoDefinition) then
          begin
            rPanel.SearchRadius :=
                Round(TT3TorpedoesOnVehicle(focused_weapon).TorpedoDefinition.FDef.Terminal_Circle_Radius
                * C_NauticalMile_To_Yards);
            rPanel.SearchDepth :=
                Round(TT3TorpedoesOnVehicle(focused_weapon).TorpedoDefinition.FDef.Default_Depth
                * C_Feet_To_Meter);
            rPanel.SafetyCeiling :=
                Round(TT3TorpedoesOnVehicle(focused_weapon).TorpedoDefinition.FDef.Acoustic_Torp_Ceiling_Depth
                * C_Feet_To_Meter);
          end;

          rPanel.ButtonLaunch := True;
          rPanel.NoTarget := True;
          rPanel.WeaponIndex := TT3TorpedoesOnVehicle(focused_weapon).InstanceIndex;
          rPanel.WeaponName := TT3TorpedoesOnVehicle(focused_weapon).InstanceName;
          rPanel.TorpType := 11;
          rPanel.Param1 := '---';
          rPanel.Param2 := '---';
          rPanel.Param3 := '---';
...
procedure TfmWeapon.chkAirDroppedUseLaunchPlatformHeadingClick(Sender: TObject);
begin
  inherited;

  if EdtADTargetTrack.Text = '' then
    Exit;
  ...
  begin
    ...
    begin
//      with TT3TorpedoesOnVehicle(focused_weapon) do begin
      if chkAirDroppedUseLaunchPlatformHeading.Checked = True then
      begin
        SimMgrClient.netSend_CmdTorpedoProperty
          (TT3TorpedoesOnVehicle(focused_weapon).ParentPlatformID, 11, 5,
           TT3TorpedoesOnVehicle(focused_weapon).InstanceIndex,
           TT3TorpedoesOnVehicle(focused_weapon).InstanceName, 1);
      end
      else
      begin
        SimMgrClient.netSend_CmdTorpedoProperty
        (TT3TorpedoesOnVehicle(focused_weapon).ParentPlatformID, 11, 5,
         TT3TorpedoesOnVehicle(focused_weapon).InstanceIndex,
         TT3TorpedoesOnVehicle(focused_weapon).InstanceName, 0);
      end;
//      end;
    end;
  end;
  ...
  begin
    with TT3BombONVehicle(focused_weapon) do begin
      ...
      begin
        ...
        begin
          {Paket Launch Torpedo}
          rBom.ParentPlatformID := ParentPlatformID;
          rBom.TargetPlatformID := TargetPlatformID;
          ...
          {Paket Synch Panel Weapon Setelah Ditembakkan}
          rPanel1.NoCubicle := 0;
          rPanel1.ParentPlatformID := ParentPlatformID;
          ...
          rPanel1.SalvoSize := 1;
          rPanel1.NoTarget := False;
          rPanel1.ButtonLaunch := False;

          SimMgrClient.netSend_CmdBombSyncPanelWeapon(rPanel1);
          Sleep(100);
          {----------------------------------------------------------------}

          SimMgrClient.netSend_CmdSetQuantity(ParentPlatformID, InstanceIndex, CORD_ID_QUANTITY, CORD_TYPE_WEAPON, (Quantity - SalvoBomb));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          lbMinesQuantity.Caption := IntToStr(Quantity - 1);
        end;
      end
      else
        ShowMessage('Cek Quantity');
...
var
  ...
  intParentPlatformID, intNoCubicle,
  IntTargetPlatformID, intbatasSR           : Integer;
...
begin
  ...
  if not simMgrClient.IsGameStart then begin
    frmTacticalDisplay.addStatus('Game Frozen');
    Exit;
  end;
  if focused_weapon = nil then begin
    frmTacticalDisplay.addStatus('Not Found Selected Weapon');
    Exit;
  end;
  if not(focused_weapon is TT3TorpedoesOnVehicle)then begin
    frmTacticalDisplay.addStatus('Selected Weapon Is Not Torpedo');
    Exit;
  end;
  if TT3TorpedoesOnVehicle(focused_weapon).WeaponStatus = wsDamaged then begin
    frmTacticalDisplay.addStatus('Selected Weapon Is Damaged');
    Exit;
  end;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  frmToteDisplay.gbSurfaceToSurface.BringToFront;
                  Break;
                end;
              end;
            end;
...
var
  strParentTrackId, strParentTrackNumber,
  strTargetTrackId, strTargetBearing      : string;

  intParentPlatformID, intNoCubicle,
  intTargetPlatformID, intbatasWH,
  intTemp, I                              : Integer;

  douBearingLaunch                        : Double;
  target                                  : TT3PlatformInstance;
  focusTarget                             : TSimObject;
begin
  inherited;
      ...
      5 : begin
            ...
            if Assigned(focused_platform) then begin
              ...
              begin
                ...
                strTargetBearing    := FormatCourse(TT3PlatformInstance(TT3DetectedTrack
              ...
              begin
                strTargetTrackId := IntToStr(TT3PlatformInstance(focused_platform).InstanceIndex);
                strTargetBearing := FormatCourse(TT3PlatformInstance(focused_platform).Course);
                IntTargetPlatformID := TT3PlatformInstance(focused_platform).InstanceIndex;
            ...
            if Assigned(simMgrClient.findDetectedTrack(target)) then
               strTargetTrackId := FormatTrackNumber(TT3DetectedTrack
                            (simMgrClient.findDetectedTrack(target)).TrackNumber)
            else
            begin
              if TT3PlatformInstance(target).TrackNumber = 0 then
                strTargetTrackId := strTargetTrackId
            ...
            if (strParentTrackId <> strTargetTrackId) and
               (strParentTrackNumber <> strTargetTrackId) then
            begin
              TargetObject := focused_platform;
              douBearingLaunch := CalcBearing(TT3PlatformInstance(FControlled).getPositionX,
                                  TT3PlatformInstance(FControlled).getPositionY,
                                  focused_platform.getPositionX, focused_platform.getPositionY);
  ...
  begin
    with TT3TorpedoesOnVehicle(focused_weapon) do
    begin
      if (EdtWHTargetTrack.Text = '') or (EdtWHTargetTrack.Text = '---') then
        Exit;
      ...
      if not Assigned(focusTarget) then
      begin
        focusTarget := simMgrClient.
          FindNonRealPlatformByID(TT3TorpedoesOnVehicle(focused_weapon).TargetPlatformID);
      end;
      ...
      case TButton(Sender).Tag of
        1 : begin
              intTemp := Round(TorpedoDefinition.FDef.Seeker_TurnOn_Range
                         * C_NauticalMile_To_Yards);
              SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 9, 3,
                         InstanceIndex, InstanceName, intTemp);
            end;
        2 : begin
              if (focusTarget = nil) or (EdtWHTargetTrack.Text = '')then
              begin
                frmTacticalDisplay.addStatus('Target is not selected!');
                exit;
              end;
              ...
              if (TorpedoPrelaunchCheck(focusTarget,focused_weapon, 9))
                  and (intbatasWH >= (StrToInt(EdtSalvoWH.Text))) then
              begin
                if (EdtSeekerRangeWH.Text = '') or (EdtSalvoWH.Text = '')
                   or (EdtLaunchBearingWH.Text = '') then
                  ShowMessage('Data Torpedo belum diisi')
                else
                begin
                  {Paket Launch Torpedo}
                  ...
                  for I := 0 to StrToInt(EdtSalvoWH.Text) - 1 do
                  begin
                    SimMgrClient.netSend_CmdLaunch_Torpedo(rMis);
                    Sleep(100);
                  end;
                  ...
                  SimMgrClient.netSend_CmdSetQuantity(ParentPlatformID,
                        InstanceIndex, CORD_ID_QUANTITY, CORD_TYPE_WEAPON,
                        (Quantity - StrToInt(EdtSalvoWH.Text)));
                  Sleep(100);

                  lblWHQuantity.Caption := IntToStr(Quantity - StrToInt(EdtSalvoWH.Text));
                end;
              end
              else
              begin
                if intbatasWH <= (StrToInt(EdtSalvoWH.Text)) then
                  ShowMessage('Cek Quantity');
              end;
            end;
      end;
...
var
  ...
  intParentPlatformID, intNoCubicle,
  IntTargetPlatformID, intBatasWG           : Integer;
...
begin
  ...
  begin
    with TT3TorpedoesOnVehicle(focused_weapon) do begin
      if (EdtWGTargetTrack.Text = '') or (EdtWGTargetTrack.Text = '---') then
        Exit;
      ...
      if (focusTarget = nil) or (EdtWGTargetTrack.Text = '') then begin
        frmTacticalDisplay.addStatus('Target is not selected!');
        exit;
      end;
      ...
      if (TorpedoPrelaunchCheck(focusTarget,focused_weapon, 8)) and (intBatasWG > 0) then
      begin
        {Paket Launch Torpedo}
        rMis.ParentPlatformID := ParentPlatformID;
        rMis.TargetPlatformID := TargetPlatformID;
        ...
        lblWGQuantity.Caption := IntToStr(Quantity-1);
      end
      else
      begin
        if intBatasWG <= 0 then
          ShowMessage('Cek Quantity')
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      case TEdit(sender).Tag of
        1 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 6, 4,
            InstanceIndex, InstanceName, value);
        2 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 6, 5,
            InstanceIndex, InstanceName, value);
        3 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 6, 6,
            InstanceIndex, InstanceName, value);
        4 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 6, 7,
            InstanceIndex, InstanceName, value);
      end;
    end;
  end;
...
procedure TfmWeapon.ActivePasiveKeyPress(Sender: TObject; var Key: Char);
var
  ValKey : set of AnsiChar;
  value : integer;
begin
  ...
  begin
    ...
    begin
      case TEdit(sender).Tag of
        1 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 10, 1,
            InstanceIndex, InstanceName, value);
        2 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 10, 2,
            InstanceIndex, InstanceName, value);
        3 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 10, 3,
            InstanceIndex, InstanceName, value);
        4 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 10, 4,
            InstanceIndex, InstanceName, value);
        5 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 10, 5,
            InstanceIndex, InstanceName, value);
      end;
    end;
  end;
...
procedure TfmWeapon.AirDroppedKeyPress(Sender: TObject; var Key: Char);
var
  ValKey : set of AnsiChar;
  value : integer;
begin
  ...
  begin
    ...
    begin
      case TEdit(sender).Tag of
        1 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 11, 1,
            InstanceIndex, InstanceName, value);
        2 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 11, 2,
            InstanceIndex, InstanceName, value);
        3 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 11, 3,
            InstanceIndex, InstanceName, value);
        4 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 11, 4,
            InstanceIndex, InstanceName, value);
      end;
    end;
  end;
...
begin
  ...
  begin
    case TSpeedButton(Sender).Tag of
      ...
      // aimpoint
      1 :  frmTacticalDisplay.fmMapWindow1.Map.CurrentTool := mtAimpoint;
...
begin
  ...
  if FControlled is TT3Vehicle then
  begin
    pi := TT3Vehicle(FControlled);

    //planed = false jika ganti senja
    if pi.Weapons.Count > 0 then
    begin
      wpn := pi.Weapons.Items[pi.LastWeaponsSelected];

      if wpn is TT3MissilesOnVehicle then
      begin
        TT3MissilesOnVehicle(wpn).Planned := False;
        btnSurfaceToSurfaceMissileWaypointsEdit.Enabled := False;
      end;
    end;

    rGun.ParentPlatformID := pi.InstanceIndex;
    wc :=  pi.getWeaponType(weaponName);
    ...
    begin
      ...
      begin
        ...
        begin
          {jk heli --> airdropped : jk bukan --> Acoustic}
          if (pi.PlatformDomain = 0) then
              wc := TWeapoonCategory(11)
          else
              wc := TWeapoonCategory(6);
        end
        else
          wc := TWeapoonCategory(TMenuItem(Sender).Tag);

        // update menu item weapon carrier vectac
        if wc = wcVectac then
          onDetectAirPlatform(pi);
      end
      else
      begin
        {jk heli --> airdropped : Prince}
        if (pi.PlatformDomain = 0) and (TMenuItem(Sender).Tag = 10)  then
          wc := TWeapoonCategory(11)
        else
          wc := TWeapoonCategory(TMenuItem(Sender).Tag);
      ...
      //weapon := pi.getWeapon(weaponName);
      DisplayTab(Byte(wc));
    end;

    weapon := pi.getWeapon(weaponName);
    for i := 0 to pi.Weapons.Count - 1 do
    begin
      wpn := TT3Weapon(pi.Weapons.Items[i]);

      if wpn.InstanceName = weaponName then
      begin
        pi.LastWeaponsSelected := i;
        Break;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          MissileOnVehicle := TT3MissilesOnVehicle(weapon);
...
begin
  ...
  begin
    ...
    weapon := TT3Weapon(vehicle.Weapons.Items[vehicle.LastWeaponsSelected]);
    edtWeaponName.Text := weapon.InstanceName;

    wc := vehicle.getWeaponType(weapon.InstanceName);
    if not(simMgrClient.ISInstructor) then
    begin
      if (Byte(wc) = 10) then
      begin
        {jk heli --> airdropped : jk bukan --> Acoustic}
        if (pi.PlatformDomain = 0) then begin
            DisplayTab(11);
            wc := wcTorpedoAirDropped;
        end
        else begin
            DisplayTab(6);
            wc := wcTorpedoActiveAcoustic;
        end;
      end
      else
        DisplayTab(Byte(wc));
    ...
    begin
      {jk heli --> airdropped : Prince}
      if (pi.PlatformDomain = 0) and (Byte(wc) = 10)  then begin
          DisplayTab(11);
          wc := wcTorpedoAirDropped;
      end
      else
    ...
    Initweapon := vehicle.getWeapon(weapon.InstanceName);
    UpdateTab(wc,Initweapon);
    ...
    begin
      ...
      weaponIdent := weapon.InstanceName;
      ...
      if not IfExistItem(pmenuWeapon, weaponIdent) then
      begin
        Item := TMenuItem.Create(self);
        Item.OnClick := OnWeaponItemSelected;
        Item.Caption := weaponIdent;
        item.Tag     := Byte(weapon.WeaponCategory);
...
begin
  ...
  case Mode of
    ...
    6 : begin
        ...
        begin
          missile.FiringMode := mfmDirect;
          rdoTacticalMissileTargetTrack.Enabled     := True;
          editTacticalMissileTargetTrack.Enabled    := True;
          editTacticalMissileTargetTrack.Color      := clWindow;
...
function TfmWeapon.TorpedoPrelaunchCheck(target, weapon : TSimObject; TipeTorpedo : Integer): boolean;
var
  range : double;
begin
  Result := True;
  ...
  {$ENDREGION}
  {--Air dropped capable n APG Cubicle, no need range dan blind zone check}
  if (TipeTorpedo = 6) or (TipeTorpedo = 11) then
  begin
    result := true;
    exit;
  end;

  {---Start pre launch check}
  with TT3TorpedoesOnVehicle(weapon) do
  begin
    {---Target range check}
    if OutsideRange(target) then
    begin
      MessageDlg('Target out of range', mtInformation, [mbOK], 0);
      result := false;
      exit;
    ...
    {---Target inside blind zone area check}
    if InsideBlindZone(target) then
    begin
  ...
  {---End pre launch check}

end;

function TfmWeapon.TorpedoTargetCheck(target, weapon: TSimObject): boolean;
var
  validTarget   : boolean;
  NrealType,
  GuideType,
...
begin
  ...
  if not(Assigned(weapon)) then exit;

  if not(Assigned(target)) then exit;

  if target is TT3DetectedTrack then begin
    NrealType   := 5;{diisi nilai 5, sebagai penanda bukan nonreal vehicle}
    PlatfDomain := TT3PlatformInstance(TT3DetectedTrack(target).TrackObject).PlatformDomain
  end
  else if target is TT3NonRealVehicle then begin
    NrealType   := TT3NonRealVehicle(target).NRPType;
    PlatfDomain := TT3PlatformInstance(target).PlatformDomain
  end
  else begin
    NrealType   := 5;
    PlatfDomain := TT3PlatformInstance(target).PlatformDomain
  ...
  TargetDomain  := TT3TorpedoesOnVehicle(weapon).TorpedoDefinition.FDef.Primary_Target_Domain;
  GuideType     := TT3TorpedoesOnVehicle(weapon).TorpedoDefinition.FDef.Guidance_Type;

  if (TT3PlatformInstance(FControlled).PlatformDomain = 0) and (GuideType = 18) then
    GuideType := 10;

  {$REGION 'NrealType'}
  { (1)nrpPoint, (2)nrpBearing, (3)nrpArea, (4)nrpSpecial }
  {$ENDREGION}

  case GuideType of
    {Straigth Torpedo}
    9  : begin
           if (target is TT3NonRealVehicle) and (not (NrealType = 1))then
             exit
           else begin
             case TargetDomain of
                {any within capability, surface / subsurface}
                0 : validTarget := (PlatfDomain = 1) or (PlatfDomain = 2);
                {surface domain}
                2 :validTarget := (PlatfDomain = 1);
                {subsurface domain}
                4 : validTarget := (PlatfDomain = 2);
             end;
           end;
         end;
    10 : begin
           if (target is TT3NonRealVehicle)then
             validTarget := True
           else begin
             case TargetDomain of
                {any within capability, surface / subsurface}
    ...
    18 : begin
           if (target is TT3NonRealVehicle) and (not ((NrealType = 1) or (NrealType = 2)))then
             exit
           else begin
             case TargetDomain of
                {any within capability, surface / subsurface}
...
procedure TfmWeapon.WakeHomingKeyPress(Sender: TObject; var Key: Char);
var
  ValKey : set of AnsiChar;
  value: Integer;
begin
  ...
  begin
    ...
    begin
      case TEdit(sender).Tag of
        1 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 9, 1,
            InstanceIndex, InstanceName, value);
        2 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 9, 2,
            InstanceIndex, InstanceName, value);
        3 : SimMgrClient.netSend_CmdTorpedoProperty(ParentPlatformID, 9, 3,
            InstanceIndex, InstanceName, value);
      end;
    end;
  end;
...
begin
  ...
  begin
    if ShowRange then
      btnStraightRunningTorpedosDisplayRangeShow.Down := true
    else
      btnStraightRunningTorpedosDisplayRangeHide.Down := true;

    if ShowBlind then
      btnStraightRunningTorpedosDisplayBlindShow.Down := true
    else
      btnStraightRunningTorpedosDisplayBlindHide.Down := true;

    case WeaponStatus of
      wsAvailable   : lblStatusStraightRunningTorpedos.Caption   := 'Available';
      wsUnavailable : lblStatusStraightRunningTorpedos.Caption   := 'Unavailable';
      wsDamaged     : lblStatusStraightRunningTorpedos.Caption   := 'Damaged';
    end;

    lblSRQuantity.Caption := IntToStr(Quantity);
    edtSRTargetTrack.Text := TT3TorpedoesOnVehicle(Focused_weapon).TargetTrack;
    lblStraightRunningTorpedosTargetIdentity.Caption := TT3TorpedoesOnVehicle(Focused_weapon).TargetIdentity;
    lblStraightRunningTorpedosTargetCourse.Caption := TT3TorpedoesOnVehicle(Focused_weapon).TargetCourse;
    lblStraightRunningTorpedosTargetGround.Caption := TT3TorpedoesOnVehicle(Focused_weapon).TargetGroundSpeed;
    lblStraightRunningTorpedosTargetDepth.Caption := TT3TorpedoesOnVehicle(Focused_weapon).TargetAltitude;
    btnLaunchSR.Enabled := TT3TorpedoesOnVehicle(Focused_weapon).ButtonLaunch;
  end;
end;
...
begin
  ...
  begin
    if ShowRange then
      sbWakeHomingDisplayRangeShow.Down := true
    else
      sbWakeHomingDisplayRangeHide.Down := true;

    if ShowBlind then
      sbWakeHomingDisplayBlindShow.Down := true
    else
      sbWakeHomingDisplayBlindHide.Down := true;

    case WeaponStatus of
      wsAvailable   : lblWakeHomingStatus.Caption   := 'Available';
      wsUnavailable : lblWakeHomingStatus.Caption   := 'Unavailable';
      wsDamaged     : lblWakeHomingStatus.Caption   := 'Damaged';
    end;

    lblWHQuantity.Caption := IntToStr(Quantity);
    EdtSalvoWH.Text := IntToStr(SalvoSize);
    EdtWHTargetTrack.Text := TargetTrack;
    lblWakeHomingTargetIdentity.Caption := TargetIdentity;
    lbWakeHomingTargetTarget.Caption := TargetBearing;
    EdtLaunchBearingWH.Text := IntToStr(round(LaunchBearing));
    EdtSeekerRangeWH.Text := FloatToStr(SeekerRange);
    btnLaunchWH.Enabled := ButtonLaunch;
  end;
end;
...
begin
  ...
  begin
    if ShowRange then
      sblWireGuidedTorpedoDisplayRangeShow.Down := true
    else
      sblWireGuidedTorpedodDisplayRangeHide.Down := true;

    if ShowBlind then
      sblWireGuidedTorpedoDisplayBlindShow.Down := true
    else
      sblWireGuidedTorpedoDisplayBlindHide.Down := true;

    case WeaponStatus of
      wsAvailable   : lblWireGuidedTorpedoStatus.Caption   := 'Available';
      wsUnavailable : lblWireGuidedTorpedoStatus.Caption   := 'Unavailable';
      wsDamaged     : lblWireGuidedTorpedoStatus.Caption   := 'Damaged';
    end;

    lblWGQuantity.Caption   := IntToStr(Quantity);
    EdtWGTargetTrack.Text := TT3TorpedoesOnVehicle(Focused_weapon).TargetTrack;
    lblWireGuidedTorpedoTargetIdentity.Caption := TT3TorpedoesOnVehicle(Focused_weapon).TargetIdentity;
    lblWireGuidedTorpedoTargetCourse.Caption := TT3TorpedoesOnVehicle(Focused_weapon).TargetCourse;
    lblWireGuidedTorpedoTargetGround.Caption := TT3TorpedoesOnVehicle(Focused_weapon).TargetGroundSpeed;
    lblWireGuidedTorpedoTargetDepth.Caption := TT3TorpedoesOnVehicle(Focused_weapon).TargetAltitude;
    btnLaunchWG.Enabled := TT3TorpedoesOnVehicle(Focused_weapon).ButtonLaunch;
  end;
end;
...
begin
  ...
  begin
    ...
    case WeaponStatus of
      wsAvailable   : lblStatusAPG.Caption   := 'Available';
      wsUnavailable : lblStatusAPG.Caption   := 'Unavailable';
      wsDamaged     : lblStatusAPG.Caption   := 'Damaged';
    end;

    lblQuantityAPG.Caption  := IntToStr(Quantity);
    EdtAPTargetTrack.Text := TargetTrack;
    EdtSalvoAP.Text := IntToStr(SalvoSize);
    EdtSearchRadiusAP.Text := FloatToStr(SearchRadius);
    EdtSearchDepthAP.Text := FloatToStr(SearchDepth);
    EdtSafetyCeilingAP.Text := FloatToStr(SafetyCeiling);
    EdtSeekerRangeAP.Text := FloatToStr(SeekerRange);
...
begin
  ...
  begin
    if ShowRange then
      btnAirDroppesDisplayRangeShow.Down := true
    else
      btnAirDroppesDisplayRangeHide.Down := true;

    if ShowBlind then
      btnAirDroppesDisplayBilndZonesShow.Down := true
    else
      btnAirDropeedBilndZonesHide.Down := true;
    ...
    lblADQuantity.Caption   := IntToStr(Quantity);

    EdtSearchRadiusAD.Text := FloatToStr(SearchRadius);
    EdtSearchDepthAD.Text := FloatToStr(SearchDepth);
    EdtSearchCeilingAD.Text := FloatToStr(SafetyCeiling);
    EdtADTargetTrack.Text := TT3TorpedoesOnVehicle(Focused_weapon).TargetTrack;
    lblAirDproppedTargetForce.Caption := TargetIdentity;
    lblAirDproppedTargetCourse.Caption := TargetCourse;
    lblAirDproppedTargetGroundSpeed.Caption := TargetGroundSpeed;
    lblAirDproppedTargetDepth.Caption := TargetAltitude;
    chkAirDroppedUseLaunchPlatformHeading.Checked := EnableLaunchBearing;
    if EnableLaunchBearing then
    begin
      EdtLaunchBearingAD.Text := '';
      EdtLaunchBearingAD.Enabled := False;
    end
    else
    begin
      EdtLaunchBearingAD.Text := IntToStr(round(LaunchBearing));
      EdtLaunchBearingAD.Enabled := True;
    end;
    if LaunchWhithoutTarget then
    begin
      chkAirDroppedLaunchWhithoutTarget.Checked := True;
    end
    else
    begin
      chkAirDroppedLaunchWhithoutTarget.Checked := False;
    end;

    btnLauchAD.Enabled := ButtonLaunch;
...
begin
  ...
  with TT3BombONVehicle(Sender) do begin
    case WeaponStatus of
    ...
    EdtBombTargetTrack.Text := TargetTrack;
