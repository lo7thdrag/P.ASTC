...
uses
  ...
  uT3Gun, DateUtils, Printers,uDynamicOverlay, System.ImageList;
...
begin
  ...
  if groupID = 0 then begin
    ...
    begin
      ...
      begin
        ...
        // visible all dynamic overlay
        for J := 0 to TT3Vehicle(aObject).ListDynamicOverlay.Count - 1 do
        begin
          DynamicOver := TT3Vehicle(aObject).ListDynamicOverlay.Items[J];
          DynamicOver.Visible := True;
        end;
      end;
  ...
  begin
    ...
    begin
      ...
      if aObject is TT3Vehicle then
      begin
        // hide all dynamic overlay first
        for J := 0 to TT3Vehicle(aObject).ListDynamicOverlay.Count - 1 do
        begin
          DynamicOver := TT3Vehicle(aObject).ListDynamicOverlay.Items[J];
          DynamicOver.Visible := False;
        end;
      end;

    end;

    grp := TT3ReplayClient(manager).Scenario.CubiclesGroupsList.GetGroupByID(groupID) as T3CubicleGroup;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // hide all dynamic overlay first
          for k := 0 to TT3Vehicle(aObject).ListDynamicOverlay.Count - 1 do
          begin
            DynamicOver := TT3Vehicle(aObject).ListDynamicOverlay.Items[k];
            DynamicOver.Visible := True;
          end;
...
begin
  ...
  for i := 0 to TT3ReplayClient(manager).Scenario.CubiclesGroupsList.Count - 1 do begin
    cub := TT3ReplayClient(manager).Scenario.CubiclesGroupsList.Items[i] as T3CubicleGroup;
