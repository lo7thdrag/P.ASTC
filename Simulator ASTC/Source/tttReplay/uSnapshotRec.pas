...
uses uDBAsset_Scripted, uDBBlind_Zone, newClassASTT, uDBAsset_Countermeasure,
  uDBAsset_Geo, uDBAsset_Deploy, uDBAsset_Reference_Point, uDataModuleTTT,
...
begin
  if obj = nil then exit ;
  s := '';
  if obj is TT3Weapon then begin
      if obj is TT3Missile then begin
          if obj is TT3Missile_SS then begin
          ...
          end else if obj is TT3Missile_SA then begin
          ...
          end else if obj is TT3Missile_AA then begin
          ...
          end else if obj is TT3Missile_AS then begin
          ...
          end else if obj is TT3Missile_LA then begin
          ...
          end else if obj is TT3Missile_HM then begin
          ...
          end else if obj is TT3Missile_TM then begin
      ...
      end else if obj is TT3Torpedo then begin
      ...
      end else if obj is TT3Mine then begin
      ...
      end else if obj is TT3Bomb then begin
      ...
      end else if obj is TT3GunShoot then begin
      ...
      end else if obj is TT3Vectac then begin
  ...
  if obj is TT3Vehicle then begin
  ...
  if obj is TT3AirBubble then begin
  ...
  if obj is TT3Jammer then begin
  ...
  if obj is TT3Sonobuoy then begin
...
begin
  ...
  begin
    ...
    if scenario.GameEnvironment <> nil then begin
    ...
    if scenario.GeoAreaDef <> nil then begin
    ...
    if scenario.GeoPoint <> nil then begin
      SetLength(GeoPoint,scenario.GeoPoint.Count);
      for I := 0 to scenario.GeoPoint.Count - 1 do begin
        mGeoAreaDef             := TGeo_Area_Def(scenario.GeoPoint[i]) ;
        if mGeoAreaDef <> nil then begin
           GeoPoint[i].FData     := mGeoAreaDef.FData ;
           if mGeoAreaDef.Point <> nil then begin
             SetLength(GeoPoint[i].Point,mGeoAreaDef.Point.Count);
             for j := 0 to mGeoAreaDef.Point.Count - 1 do begin
                mGeoAreaPoint        := TGeo_Area_Point(mGeoAreaDef.Point[j]) ;
                if mGeoAreaPoint <> nil then GeoPoint[i].Point[j] := mGeoAreaPoint.FData ;
             end;
           end;
           if mGeoAreaDef.Event <> nil then begin
             SetLength(GeoPoint[i].Event,mGeoAreaDef.Event.Count);
             for j := 0 to mGeoAreaDef.Point.Count - 1 do begin
                mGeoAreaEvent        := TConditional_Event(mGeoAreaDef.Event[j]) ;
                if mGeoAreaEvent <> nil then GeoPoint[i].Event[j] := mGeoAreaEvent.FData ;
             end;
          end;
        end;
      end;
    ...
    if scenario.Overlay_Definition <> nil then begin
        Overlay_Definition        := scenario.Overlay_Definition.FData;
        Overlay_DefinitionObjFlag := isExist ;
    end;
...
//        Resource_Overlay_Mapping  := scenario.Resource_Overlay_Mapping.FData;
          ...
          for I := 0 to scenario.OverlayTemplateFromDB.Count - 1 do begin
            ...
            if mResAllocation <> nil then begin
              ...
              Overlay_Mapping[i].IsOverlayShow:= mResAllocation.IsOverlayShow ;
              Overlay_Mapping[i].IsOverlayEdit:= mResAllocation.IsOverlayEdit ;
              Overlay_Mapping[i].IsCubicle:= mResAllocation.IsCubicle ;
              Overlay_Mapping[i].FCubicleNumbr:= mResAllocation.FCubicleNumbr ;
            end;
          end;
        Overlay_MappingObjFlag := isExist ;
    end;

    if scenario.AssetDeployment <> nil then begin
      AssetDeployment           := scenario.AssetDeployment.FData ;
      AssetDeploymentObjFlag    := isExist;
    end;
    MapGeosetName             := scenario.MapGeosetName;
    //predefPatternName         := scenario.predefPatternName;
    overlayName               := scenario.overlayName;
    for I := low(scenario.allOverlayNames) to high(scenario.allOverlayNames) do begin
    ...
    begin
      fmList := Scenario.Formation_List[i];
      if fmList <> nil then begin
          ...
          for j := 0 to fmList.Count - 1 do begin
            ...
            if fmDef <> nil then begin
                Formation_List.FFormList[i].FForm[j].FFormation_Def := fmDef.FFormation_Def ;
                Formation_List.FFormList[i].FForm[j].FForm_Assign   := fmDef.FForm_Assign ;
            end;
          end;
      end;
    end;
    ...
    if scenario.Ref_Point <> nil then begin
      Ref_Point.FData           := scenario.Ref_Point.FData;
      Ref_Point.Symbol_Name     := scenario.Ref_Point.Symbol_Name;
      Ref_Point.Symbol_Color    := scenario.Ref_Point.Symbol_Color;
      Ref_Point.IsSelected      := scenario.Ref_Point.IsSelected;
      Ref_PointObjFlag          := isExist;
    end;
    // reff getAllReference_Point
    if scenario.rpList <> nil then  begin
        SetLength(rpList,scenario.rpList.Count);
        for i := 0 to scenario.rpList.Count - 1 do begin
             mRefPoint :=  TReference_Point(scenario.rpList[i]);
...
begin
  ...
  begin
    ...
    if Overlay_DefinitionObjFlag = isExist then begin
      if scenario.Overlay_Definition = nil then
         scenario.Overlay_Definition := uDBAsset_GameEnvironment.TOverlay_Definition.Create ;
      scenario.Overlay_Definition.FData        := Overlay_Definition;
    end;
...
//    dmTTT.getAllRuntimePlatform(scenario.ResourceAllocIndex,scenario.RuntimePlatformLibrary); // diubah prosesnya
    ...
    begin
       ...
       begin
          fmDef    := TFormation.Create;
          fmDef.FFormation_Def := Formation_List.FFormList[I].FForm[j].FFormation_Def;
          fmDef.FForm_Assign   := Formation_List.FFormList[I].FForm[j].FForm_Assign;
    ...
    if Ref_PointObjFlag = isExist then begin
      if scenario.Ref_Point = nil then
         scenario.Ref_Point := TReference_Point.Create;
      scenario.Ref_Point.FData           := Ref_Point.FData;
      scenario.Ref_Point.Symbol_Name     := Ref_Point.Symbol_Name;
      scenario.Ref_Point.Symbol_Color    := Ref_Point.Symbol_Color;
      scenario.Ref_Point.IsSelected      := Ref_Point.IsSelected;
    end;

    // reff getAllReference_Point

    if scenario.rpList = nil then
        scenario.rpList := TList.Create;
    for i := Low(rpList) to High(rpList) do begin
       mRefPoint :=  TReference_Point.Create;
       mRefPoint.FData := rpList[i].FData ;
       scenario.rpList.Add(mRefPoint);
    end;
...
begin
  ...
  for i := 0 to sim.itemCount - 1 do begin
    obj := sim.getObject(i);
    if obj is TT3Vehicle then begin
        ve := TT3Vehicle(obj);
        idx := ve.InstanceIndex  ;
        if lRec.GetVehicleDataFromVehicleID(idx,recSSVehicle) then begin
          recSSVehicle := lRec.FRVehicle[i] ;
          ve.SetSnapshotData(recSSVehicle);
          /// --------  device
          for j:= 0 to ve.Devices.Count - 1 do begin
              obj := ve.Devices[j] ;
          ...
          /// --------  device
          for j:= 0 to ve.Weapons.Count - 1 do begin  /// -------- Weapons
              obj := ve.Weapons[j] ;
    ...
    end else // end if obj is TT3Vehicle then begin
