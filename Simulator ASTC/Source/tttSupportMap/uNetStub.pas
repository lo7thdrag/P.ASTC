...
procedure TNetStub.SetOnLogStr(const Value: TGetStrProc);
begin
  FOnLogStr := Value;
end;
...
begin
  if Assigned(OnLogStr) then
    OnLogStr('Game Control : ' +  S + ' connected');
end;

procedure TNetStubGC.FNetStubGC_OnClientDisConnect(const S: string);
begin
  if Assigned(OnLogStr) then
    OnLogStr('Game Control : ' +  S + ' disconnected');
end;
...
procedure TNetStubGC.SetOnLogStr(const Value: TGetStrProc);
begin
  FOnLogStr := Value;
end;
