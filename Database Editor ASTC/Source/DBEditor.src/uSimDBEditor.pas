...
uses
  Controls, Classes, tttData, uDBScenario, SysUtils, uDBFormation,
  uDBAsset_GameEnvironment, uDBGame_Defaults, uT3SimManager, uDBAsset_Weapon,
  newClassASTT, uDBAssetObject, uDBAsset_Satellite,
  uDBAsset_MotionCharacteristics, uDBAsset_Radar, uDBAsset_Sensor,
  uDBAsset_Sonobuoy, uDBAsset_Sonar, uDBAsset_Vehicle, uDBNote_Storage;
  ...
  //satellite
  procedure getSatellite_Definition(const id: integer;
    var rec: TSatellite_Definition);
  procedure insertSatellite_Def(rec: TSatellite_Definition);
  procedure updateSatellite_Def(var rec: TSatellite_Definition;
    const id: string);
  procedure deleteSatellite_Def(id : Integer);

  //motion
  procedure getMotion_Characteristics(const id: integer;
    var rec: TMotion_Characteristics);
  ...
  procedure getAllSonar(const id:integer;var aRec:TList;
    var rec: TSonar_On_Board);
  ...
  //sonobuoy
  procedure getAllSonobuoy (const id_vec,id:integer;var slist: TList;
    var rec:TSonobuoy_On_Board);
...
uses
  ...
  uDBAssets_SubAreaEnviroDefinition, uDBEditSetting;//,uAvailableScenario;
...
//  mulai load scenario
  ...
  LoadFF_AppDBSetting(vSettingFile, vAppDBSetting);
...
procedure getAllMotion_Characteristics(mList : TList);
begin
  dmTTT.GetAllMotionCharacteristicDef(mList);
end;
