...
type
  TfrmChaffAssets = class(TForm)
    pgcChaffMountData: TPageControl;
    tsGeneral: TTabSheet;
    grpChaffMount: TGroupBox;
    btnEditChaffMount: TButton;
    lblMinDelay: TLabel;
    lbl1: TLabel;
    lblMaxNumberChaff: TLabel;
    ...
    grpLauncher3: TGroupBox;
    chkLauncher3: TCheckBox;
    btnLauncher3: TButton;
    grpLauncher4: TGroupBox;
    chkLauncher4: TCheckBox;
    btnLauncher4: TButton;
    tsSeduction: TTabSheet;
    chkCapable: TCheckBox;
    lblTimeWeaponImpact: TLabel;
    lbl2: TLabel;
    grpProbabilitySuccess: TGroupBox;
...
begin
  frmChaffOnBoardOnBoardPickList := TfrmChaffOnBoardOnBoardPickList.Create(Self);
  try
    with frmChaffOnBoardOnBoardPickList do
    begin
      SelectedVehicle := FSelectedVehicle;
      ShowModal;
    end;

    btnApply.Enabled := frmChaffOnBoardOnBoardPickList.AfterClose;
    btnCancel.Enabled := not frmChaffOnBoardOnBoardPickList.AfterClose;

  finally
    frmChaffOnBoardOnBoardPickList.Free;
  end;
end;

procedure TfrmChaffAssets.chkCapableClick(Sender: TObject);
begin
  lbl2.Enabled  := chkCapable.Checked;
...
begin
  edtProbabilitySuccess.Text := IntToStr(TTrackBar(Sender).Position);
end;
