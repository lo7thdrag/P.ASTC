object frmVehicleResourceAllocationPickList: TfrmVehicleResourceAllocationPickList
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsToolWindow
  Caption = '   Vehicle'
  ClientHeight = 408
  ClientWidth = 754
  Color = clBtnShadow
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 14
  object pnlMain: TPanel
    Left = 0
    Top = 0
    Width = 754
    Height = 408
    Align = alClient
    BevelOuter = bvNone
    BorderWidth = 10
    Color = 2499101
    ParentBackground = False
    TabOrder = 1
    ExplicitHeight = 418
    object imgExercise: TImage
      Left = 12
      Top = 8
      Width = 313
      Height = 30
      Cursor = crHandPoint
      Picture.Data = {
        }
      Stretch = True
    end
    object Label1: TLabel
      Left = 22
      Top = 16
      Width = 124
      Height = 14
      Cursor = crHandPoint
      Caption = 'List of available Vehicle'
      Color = clHighlight
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Image1: TImage
      Left = 430
      Top = 8
      Width = 313
      Height = 30
      Cursor = crHandPoint
      Picture.Data = {
        }
      Stretch = True
    end
    object Label2: TLabel
      Left = 439
      Top = 16
      Width = 206
      Height = 14
      Cursor = crHandPoint
      Caption = 'List of Vehicle On Resource Allocation'
      Color = clHighlight
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label3: TLabel
      Left = 17
      Top = 341
      Width = 53
      Height = 16
      Caption = 'Search : '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object btnAdd: TButton
      Left = 346
      Top = 144
      Width = 62
      Height = 25
      Caption = 'Add'
      TabOrder = 0
      OnClick = btnAddClick
    end
    object btnRemove: TButton
      Left = 346
      Top = 175
      Width = 62
      Height = 25
      Caption = 'Remove'
      TabOrder = 1
      OnClick = btnRemoveClick
    end
    object lbAllVehicleDef: TListBox
      Left = 12
      Top = 44
      Width = 313
      Height = 286
      ItemHeight = 14
      TabOrder = 2
      OnClick = lbAllVehicleDefClick
      OnDblClick = btnAddClick
    end
    object lbAllVehicleOnRA: TListBox
      Left = 430
      Top = 44
      Width = 313
      Height = 286
      ItemHeight = 14
      TabOrder = 3
      OnClick = lbAllVehicleOnRAClick
      OnDblClick = btnRemoveClick
    end
    object btnEdit: TButton
      Left = 668
      Top = 336
      Width = 75
      Height = 25
      Caption = 'Edit'
      TabOrder = 4
      OnClick = btnEditClick
    end
    object edtSearch: TEdit
      Left = 78
      Top = 336
      Width = 247
      Height = 25
      AutoSize = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      OnKeyPress = edtSearchKeyPress
    end
  end
  object pnlButton: TPanel
    Left = 0
    Top = 367
    Width = 754
    Height = 43
    BevelOuter = bvNone
    Color = 2499101
    ParentBackground = False
    TabOrder = 0
    object btnClose: TButton
      Left = 668
      Top = 3
      Width = 75
      Height = 25
      Caption = 'Close'
      TabOrder = 0
      OnClick = btnCloseClick
    end
  end
end
