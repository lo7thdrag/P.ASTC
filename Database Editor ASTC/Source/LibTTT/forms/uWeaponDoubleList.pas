...
{$R *.dfm}
uses ufrmAvailableVehicle,uDataModuleTTT,uDBAsset_Weapon;
...
begin
  frmAvailableVehicle.edtFilterWeaponOnBoard.Text := '[None]';
  Close;
end;
...
begin
  ...
  begin
    ...
    frmAvailableVehicle.edtFilterWeaponOnBoard.Text := lbWeaponSel.Items.Strings[0];
  end
  else begin
    idWeapon := '';
    frmAvailableVehicle.edtFilterWeaponOnBoard.Text := '[None]';
  ...
  Close;
  frmAvailableVehicle.chkWeaponClick(Sender);
end;
...
procedure TfWeaponDoubleList.FormCreate(Sender: TObject);
begin
  wlist := TList.Create;
end;
...
begin
  ...
  begin
    ...
    begin
      lbAllWeapon.Items.Add(TMine_On_Board(wList[i]).FMine_Def.Mine_Identifier);
      lbTempIdAll.Items.Add(IntToStr(TMine_On_Board(wList[i]).FMine_Def.Mine_Index));
    end
    else if frmAvailableVehicle.cbbFilterWeaponType.ItemIndex = 1 then
    begin
      lbAllWeapon.Items.Add(TMissile_On_Board(wList[i]).FDef.Class_Identifier);
      lbTempIdAll.Items.Add(IntToStr(TMissile_On_Board(wList[i]).FDef.Missile_Index));
    end
    else begin
      lbAllWeapon.Items.Add(TTorpedo_On_Board(wList[i]).FDef.Class_Identifier);
      lbTempIdAll.Items.Add(IntToStr(TTorpedo_On_Board(wList[i]).FDef.Torpedo_Index));
    end
