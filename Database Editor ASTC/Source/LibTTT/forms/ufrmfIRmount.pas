...
procedure TfrmfIRmount.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;

procedure TfrmfIRmount.FormShow(Sender: TObject);
begin
...
procedure TfrmfIRmount.btnOKClick(Sender: TObject);
begin
...
procedure TfrmfIRmount.btnApplyClick(Sender: TObject);
begin
...
procedure TfrmfIRmount.btnCancelClick(Sender: TObject);
begin
  AfterClose := False;
  Close;
end;

function TfrmfIRmount.CekInput: Boolean;
begin
  Result := False;
...
procedure TfrmfIRmount.UpdateInfraredDecoyData;
begin
  with FSelectedInfraredDecoy do
  begin
    if FData.Infrared_Decoy_Instance_Index = 0 then
...
function TfrmfIRmount.GetNumberOfKoma(s: string): Boolean;
var
  a, i : Integer;
begin
  Result := False;
...
procedure TfrmfIRmount.edtNumeralKeyPress(Sender: TObject; var Key: Char);
var
  value : Double;
begin
...
procedure TfrmfIRmount.edtChange(Sender: TObject);
begin
  btnApply.Enabled := True;
end;

procedure TfrmfIRmount.ValidationFormatInput;
var
  i: Integer;
  value : Double;
